...
void FEditorVisibility::MakePortals( INT iNode )
{
	...
	while( iNode != INDEX_NONE )
	{
		...
		if( (Surf.PolyFlags[0] & PF_Portal) && GEditor->bspNodeToFPoly( Model, iNode, &Poly ) )
#else
...
void FEditorVisibility::FormZonesFromLeaves()
{
...
#if 1 //NEW: Fix
	// By Mark Poesch:
	// Legend's zoning algorithm to help control merged zones
	...
	for( int iActor=0; iActor<Level->Actors.Num(); iActor++ )
	{
		...
		if( Actor && !Actor->IsA(ALevelInfo::StaticClass()) )
		{
			...
			debugf( NAME_Log, TEXT("iActor: %2d (AZoneInfo) marked iZone: %3d valid"), iActor, iZone );
		}
	}
	...
	for( i=0; i<Model->Leaves.Num(); i++ )
	{
		if( Model->Leaves(i).iZone >= NumZones )
		{
			...
			if( ValidZones[ Model->Leaves(i).iZone ] ) {
				iZone = NumZones;
				debugf( NAME_Log, TEXT("Moving zone %3d to zone %2d"), Model->Leaves(i).iZone, iZone + 1 );
			} else {
				iZone = -1;
				debugf( NAME_Log, TEXT("Moving zone %3d to the bad place (63)"), Model->Leaves(i).iZone );
			}
			...
			if( iZone != -1 ) {
				...
				if( NumZones >= 62 )
				{
					debugf( NAME_Log, TEXT("WARNING: NumZones constrained to 62 -- zones merged!") );
					NumZones = 0;
				}
			}
		}
	...
	debugf( NAME_Log, TEXT("WOT ZONING ACTIVE: Found %i zones"), NumZones );
#else
	// Renumber the leaves.
	INT NumZones=0;
	for( INT i=0; i<Model->Leaves.Num(); i++ )
