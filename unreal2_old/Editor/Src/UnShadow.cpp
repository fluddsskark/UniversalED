...
#if 1 //NEW: Lighting
	INT ComputeAllLightVisibility( UBOOL Selected, INT xRes=128, INT yRes=128 );
	void LightBspSurf( AMover* Mover, INT iSurf, INT iPoly, UBOOL Debug=0 );
#else
...
//
#if 1 //NEW: Lighting
INT FMeshIlluminator::ComputeAllLightVisibility( UBOOL Selected, INT xRes, INT yRes )
#else
...
#endif
{
...
#if 1 //NEW: Lighting
	Viewport->OpenWindow( 0, 1, xRes, yRes, INDEX_NONE, INDEX_NONE );
#else
	...
	for( INT i=0; i<Level->Actors.Num(); i++ )
	{
		if( (i&15)==0 )
#if 1 //NEW: Fix -- MERGED
			GWarn->StatusUpdatef( i, Level->Actors.Num(), TEXT("%s"), TEXT("Computing visibility") );
#else
		...
		if( Actor && Actor->LightType!=LT_None && (Actor->bStatic||Actor->bNoDelete) )
		{
			...
			if( DoLight )
			{
				...
				for( INT i=0; i<iSurfs.Num(); i++ )
				{
					...
					&&	(Actor->bSpecialLit ? (Poly.PolyFlags[0]&PF_SpecialLit) : !(Poly.PolyFlags[0]&PF_SpecialLit))
#else
...
//
#if 1 //NEW: Lighting
void FMeshIlluminator::LightBspSurf( AMover* Mover, INT iSurf, INT iPoly, UBOOL Debug )
#else
...
#endif
{
...
#if 1 //NEW: PolyFlagsEx
		SetupIndex( Index, Poly.PolyFlags[0] | (Mover->bDynamicLightMover ? PF_LowShadowDetail : 0), MinU, MinV, MaxU, MaxV );
#else
	...
	else
	{
		...
		unguard;
#if 1 //NEW: PolyFlagsEx
		SetupIndex( Index, Surf.PolyFlags[0], MinU, MinV, MaxU, MaxV );
#else
...
#if UNALIGNED_LIGHT
#if 1 //NEW: PolyFlagsEx
			if( Surf.PolyFlags[0] & PF_BrightCorners )
#else
					...
					for( INT SubU=UCounter; SubU<Index->UClamp && M; SubU++,M=M<<1 )
					{
						if( FDistSquared(Actor->Location,Vertex) < SqRadius )
...
#if 1 //NEW: Lighting
							{
								...
								DidHit=1;
#if 1 //NEW: PolyFlagsEx
								if( Debug && Surf.PolyFlags[0] & PF_Selected )
#else
								if( Debug && Surf.PolyFlags & PF_Selected )
#endif
								{
									UClass* Class = (UClass*)Level->StaticLoadObject( UClass::StaticClass(), NULL, TEXT("Legend.LineDot3"), NULL, LOAD_NoWarn, NULL );
									Level->SpawnActor( Class, NAME_None, NULL, NULL, Vertex, Normal.Rotation(), NULL, 1 );
								}
							}
							else
							{
...
#endif
								{
									UClass* Class = (UClass*)Level->StaticLoadObject( UClass::StaticClass(), NULL, TEXT("Legend.LineDot2"), NULL, LOAD_NoWarn, NULL );
									Level->SpawnActor( Class, NAME_None, NULL, NULL, Vertex, Normal.Rotation(), NULL, 1 );
								}
...
void FMeshIlluminator::BuildSurfList( INT iNode )
{
	...
	{
#if 1 //NEW: PolyFlagsEx
		if( !(Surf.PolyFlags[0] & PF_NoShadows) )
#else
...
#if 1 //NEW: Lighting
void UEditorEngine::shadowIlluminateBsp( ULevel* Level, INT Selected, INT xRes, INT yRes, UBOOL ShowMaps )
#else
...
#if 1 //NEW: PolyFlagsEx
						if( !(Poly.PolyFlags[0] & PF_NoShadows) )
#else
...
#if 1 //NEW: Lighting
		Illum.NumLights = Illum.ComputeAllLightVisibility( Selected, xRes, yRes );
#else
...
#if 1 //NEW: Lighting
				Illum.LightBspSurf( NULL, i, 0, ShowMaps );
#else
...
#if 1 //NEW: Lighting
						Illum.LightBspSurf( Actor, Poly.iLink, j, ShowMaps );
#else
