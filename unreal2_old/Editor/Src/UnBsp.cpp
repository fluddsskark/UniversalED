...
)
{
	...
	if( EdPoly->iLink == Model->Surfs.Num() )
	{
		...
		Surf->PolyFlags[0] 	= EdPoly->PolyFlags[0] & ~PF_NoAddToBSP;
		Surf->PolyFlags[1] 	= EdPoly->PolyFlags[1];
#else
	...
	// Set NodeFlags.
#if 1 //NEW: PolyFlagsEx
	if( Surf->PolyFlags[0] & PF_NotSolid              ) NodeFlags |= NF_NotCsg;
	if( Surf->PolyFlags[0] & (PF_Invisible|PF_Portal) ) NodeFlags |= NF_NotVisBlocking;
	if( Surf->PolyFlags[0] & PF_Masked                ) NodeFlags |= NF_ShootThrough;
#else
...
)
{
	...
	float	PortalBias = ( ( Balance >> 8 ) & 0xFF ) / 100.0;	// grab half of the first word from Balance (implemented as _WORD in UnEdSrv.cpp) to recover "PortalBias"
	Balance &= 0xFF;								// keep only the low byte to recover "Balance"
	debugf( NAME_DevBSP, TEXT("Balance=%d PortalBias=%f"), Balance, PortalBias );
#endif
...
#if 1 //NEW: PolyFlagsEx
		if( !(PolyList[i]->PolyFlags[0] & PF_AddLast) )
#else
	...
	for( i=0; i<NumPolys; i+=Inc )
	{
		...
		do
		{
			...
			&& ( (Poly->PolyFlags[0] & PF_AddLast) && !(Poly->PolyFlags[0] & PF_Portal) )
			&& !AllSemiSolids );
#else
#if 1 //LEGEND
		} while( Index<(i+Inc) && Index<NumPolys 
...
#if 1 //NEW: PolyFlagsEx
					if( !(OtherPoly->PolyFlags[0] & PF_Portal) )
#else
#if 1 //LEGEND
...
#else
			Score -= ( 100.0 - float(Balance) ) * Splits; // ignore the split polys -- portals should be selected for cut!
#endif
		}
	...
	//debugf( TEXT("  Score = %f, Flags = %08X"), BestScore, Best->PolyFlags ); //NEW: Logging
	check(Best);
	return Best;
	unguard;
}
...
#if 0 //NEW: Split List
void DumpSplitInfo( INT IndentLevel, FPoly* SplitPoly )
{
	static FString Padding( TEXT("                                                                                                                                                                                                                                                                ") );

	GLog->Logf( TEXT("%s %d (%X)"), *Padding.Left( IndentLevel * 2 ), SplitPoly->iLink, SplitPoly->PolyFlags );
}
#endif
//
...
#if 0 //NEW: Split List
	DumpSplitInfo( IndentLevel, SplitPoly );
#endif
...
#if 1 //NEW
	debugf( NAME_DevBSP, TEXT("bspBuild built %i convex polys into %i nodes"), OriginalPolys, Model->Nodes.Num() );
#else
...
#if 1 //NEW: PolyFlagsEx
					&&	OtherPoly->PolyFlags[0] == EdPoly->PolyFlags[0]
					&&	OtherPoly->PolyFlags[1] == EdPoly->PolyFlags[1]
#else
...
{
	guard(UEditorEngine::bspUnlinkPolys);
	Brush->Modify();
	Brush->Linked = 1;
	for( INT i=0; i<Brush->Polys->Element.Num(); i++ )
		Brush->Polys->Element(i).iLink = i;
...
void UEditorEngine::bspMergeCoplanars( UModel* Model, UBOOL RemapLinks, UBOOL MergeDisparateTextures )
{
	...
	for( INT i=0; i<Model->Polys->Element.Num(); i++ )
#if 1 //NEW: PolyFlagsEx
		Model->Polys->Element(i).PolyFlags[0] &= ~PF_EdProcessed;
#else
...
#if 1 //NEW: PolyFlagsEx
		if( EdPoly->NumVertices>0 && !(EdPoly->PolyFlags[0] & PF_EdProcessed) )
#else
...
#endif
		{
...
#if 1 //NEW: PolyFlagsEx
			EdPoly->PolyFlags[0]    |= PF_EdProcessed;
#else
			...
			for( INT j=i+1; j<Model->Polys->Element.Num(); j++ )
	        {
				...
				if( OtherPoly->iLink == EdPoly->iLink )
				{
					...
					{
#if 1 //NEW: PolyFlagsEx
						OtherPoly->PolyFlags[0] |= PF_EdProcessed;
#else
...
#if 1 //NEW: PolyFlagsEx
			if( !(EdPoly->PolyFlags[0] & PF_Semisolid) )
#else
...
#if 1 //NEW: PolyFlagsEx
			if( EdPoly->PolyFlags[0] & PF_EdCut )
#else
...
#if 1 //NEW: PolyFlagsEx
	EdPoly->PolyFlags[0] = Poly.PolyFlags[0] & ~(PF_EdCut | PF_EdProcessed | PF_Selected | PF_Memorized);
	EdPoly->PolyFlags[1] = Poly.PolyFlags[1];
#else
...
)
{
	...
	if( ReallyBig )
	{
		...
		GWarn->BeginSlowTask( Descr, 1, 0 );
	}

	// Transform original brush poly into same coordinate system as world
	// so Bsp filtering operations make sense.
	if( ReallyBig ) GWarn->StatusUpdatef(0,0,TEXT("%s"),TEXT("Transforming"));
	for( i=0; i<Brush->Polys->Element.Num(); i++ )
	{
		// Set texture the first time.
		if( !Brush->Polys->Element(i).Texture )
			Brush->Polys->Element(i).Texture = CurrentTexture;
		...
		// Update its flags.
#if 1 //NEW: PolyFlagsEx
		DestEdPoly.PolyFlags[0] = (DestEdPoly.PolyFlags[0] | PolyFlags) & ~NotPolyFlags;
#else
	...
	if( ReallyBig ) GWarn->StatusUpdatef( 0, 0, TEXT("%s"), TEXT("Filtering brush") );
	...
	if( Model->Nodes.Num() && !(PolyFlags & (PF_NotSolid | PF_Semisolid)) )
	{
		...
		if( ReallyBig ) GWarn->StatusUpdatef( 0, 0, TEXT("%s"), TEXT("Building Bsp") );
		
		bspBuild( TempModel, BSP_Lame, 0, 1, 0 );
		
		if( ReallyBig ) GWarn->StatusUpdatef( 0, 0, TEXT("%s"), TEXT("Filtering world") );
		GModel = Brush;
	...
	if( CSGOper==CSG_Intersect || CSGOper==CSG_Deintersect )
	{
		if( ReallyBig ) GWarn->StatusUpdatef( 0, 0, TEXT("%s"), TEXT("Adjusting brush") );
	...
	{
		if( ReallyBig )
			GWarn->StatusUpdatef( 0, 0, TEXT("%s"), TEXT("Merging") );
