...
void UEditorEngine::polySetAndClearPolyFlags(UModel *Model, DWORD SetBits, DWORD ClearBits,int SelectedOnly, int UpdateMaster)
{
	...
	for( INT i=0; i<Model->Surfs.Num(); i++ )
	{
		...
		if( !SelectedOnly || (Poly.PolyFlags[0] & PF_Selected) )
		{
			DWORD NewFlags = (Poly.PolyFlags[0] & ~ClearBits) | SetBits;
			if( NewFlags != Poly.PolyFlags[0] )
			{
				Model->ModifySurf( i, UpdateMaster );
				Poly.PolyFlags[0] = NewFlags;
				if( UpdateMaster )
...
#if 1 //NEW: PolyFlagsEx
//
// Sets and clears all Bsp node flags for licensee-defined PolyFlags
//
void UEditorEngine::polySetAndClearPolyFlags2(UModel *Model, DWORD SetBits, DWORD ClearBits,int SelectedOnly, int UpdateMaster)
{
	guard(UEditorEngine::polySetAndClearPolyFlags2);
	for( INT i=0; i<Model->Surfs.Num(); i++ )
	{
		FBspSurf& Poly = Model->Surfs(i);
		if( !SelectedOnly || (Poly.PolyFlags[0] & PF_Selected) )
		{
			DWORD NewFlags = (Poly.PolyFlags[1] & ~ClearBits) | SetBits;
			if( NewFlags != Poly.PolyFlags[1] )
			{
				Model->ModifySurf( i, UpdateMaster );
				Poly.PolyFlags[1] = NewFlags;
				if( UpdateMaster )
					polyUpdateMaster( Model, i, 0, 0 );
			}
		}
...
#if 1 //NEW: PolyFlagsEx
			MasterEdPoly.PolyFlags[0] = Poly.PolyFlags[0] & ~(PF_NoEdit);
			MasterEdPoly.PolyFlags[1] = Poly.PolyFlags[1];
#else
...
void UEditorEngine::polyFindByFlags(UModel *Model, DWORD SetBits, DWORD ClearBits, POLY_CALLBACK Callback)
	...
	for (INT i=0; i<Model->Surfs.Num(); i++)
		{
#if 1 //NEW: PolyFlagsEx
		if (((Poly->PolyFlags[0]&SetBits)!=0) || ((Poly->PolyFlags[0]&~ClearBits)!=0))
#else
...
void UEditorEngine::polyResetSelection(UModel *Model)
	...
	for (INT i=0; i<Model->Surfs.Num(); i++)
		...
		FBspSurf *Poly = &Model->Surfs(i);
#if 1 //NEW: PolyFlagsEx
		Poly->PolyFlags[0] |= ~(PF_Selected | PF_Memorized);
#else
	...
	for (INT i=0; i<Model->Surfs.Num(); i++)
...
#if 1 //NEW: PolyFlagsEx
		if (SelectedPoly->PolyFlags[0] & PF_Selected)
#else
...
#endif
		{
			...
			for (INT j=0; j<Model->Surfs.Num(); j++)
				...
				//
#if 1 //NEW: PolyFlagsEx
				if (FCoplanar(*Base,*Normal,*SelectedBase,*SelectedNormal) && (!(Poly->PolyFlags[0] & PF_Selected)))
#else
...
void UEditorEngine::polySelectMatchingBrush(UModel *Model)
	...
	for (i=0; i<Model->Surfs.Num(); i++)
		...
		if( Poly->Actor->Brush )
			{
#if 1 //NEW: PolyFlagsEx
			if ((GFlags1[Poly->Actor->Brush->GetIndex()])&&(!(Poly->PolyFlags[0]&PF_Selected)))
#else
...
void UEditorEngine::polySelectMatchingTexture(UModel *Model)
	...
	for (i=0; i<Model->Surfs.Num(); i++)
		...
		FBspSurf *Poly = &Model->Surfs(i);
#if 1 //NEW: PolyFlagsEx
		if (Poly->Texture && (Poly->PolyFlags[0]&PF_Selected)) GFlags1[Poly->Texture->GetIndex()]=1;
#else
	...
	for (i=0; i<Model->Surfs.Num(); i++)
		...
		FBspSurf *Poly = &Model->Surfs(i);
#if 1 //NEW: PolyFlagsEx
		if (Poly->Texture && (GFlags1[Poly->Texture->GetIndex()]) && (!(Poly->PolyFlags[0]&PF_Selected)))
#else
...
#if 1 //NEW: PolyFlagsEx
		else if (Blank & !Poly->Texture) Poly->PolyFlags[0] |= PF_Selected;
#else
...
void UEditorEngine::polyMemorizeSet(UModel *Model)
	...
	for (INT i=0; i<Model->Surfs.Num(); i++)
		...
		if (Poly->PolyFlags & PF_Selected) 
			...
			{
#if 1 //NEW: PolyFlagsEx
			if (!(Poly->PolyFlags[0] & PF_Memorized))
#else
...
void UEditorEngine::polyXorSet(UModel *Model)
	...
	for (INT i=0; i<Model->Surfs.Num(); i++)
		...
		FBspSurf *Poly = &Model->Surfs(i);
#if 1 //NEW: PolyFlagsEx
		Flag1 = (Poly->PolyFlags[0] & PF_Selected ) != 0;
		Flag2 = (Poly->PolyFlags[0] & PF_Memorized) != 0;
#else
...
void UEditorEngine::polyIntersectSet(UModel *Model)
	...
	for (INT i=0; i<Model->Surfs.Num(); i++)
		...
		FBspSurf *Poly = &Model->Surfs(i);
#if 1 //NEW: PolyFlagsEx
		if ((Poly->PolyFlags[0] & PF_Memorized) && !(Poly->PolyFlags[0] & PF_Selected))
#else
...
void PolysTopicHandler::Get( ULevel* Level, const TCHAR* Item, FOutputDevice& Ar )
{
	...
	for( INT i=0; i<Level->Model->Surfs.Num(); i++ )
	{
		...
		{
#if 1 //NEW: PolyFlagsEx
			OnFlags  &=  Poly->PolyFlags[0];
			OffFlags &= ~Poly->PolyFlags[0];
#else
