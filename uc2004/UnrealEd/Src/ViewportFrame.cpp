...
void WVFToolBar::OnRightButtonUp()
{
	...
	if		(ShowFilter==(SHOW_Actors | SHOW_ActorIcons))
		CheckMenuItem( submenu, ID_ActorsIcons, MF_CHECKED );
	else if (ShowFilter==(SHOW_Actors | SHOW_ActorRadii))
		CheckMenuItem( submenu, ID_ActorsRadii, MF_CHECKED );
	else if (ShowFilter==(SHOW_Actors | SHOW_ActorInfo ))
		CheckMenuItem( submenu, ID_ActorInfo, MF_CHECKED );
	else if	(ShowFilter==SHOW_Actors)
		CheckMenuItem( submenu, ID_ActorsShow, MF_CHECKED );
...
void WViewportFrame::OnKeyUp( WPARAM wParam, LPARAM lParam )
{
	...
	switch( wParam )
	{
		//scion superville: Movement keys for brushes and vertices
		case VK_LEFT:
			GUnrealEd->Bump(Viewport, -1, 0);
			break;
		case VK_RIGHT:
			GUnrealEd->Bump(Viewport, 1, 0);
			break;
		case VK_UP:
			GUnrealEd->Bump(Viewport, 0, 1);
			break;
		case VK_DOWN:
			GUnrealEd->Bump(Viewport, 0, -1);
			break;
