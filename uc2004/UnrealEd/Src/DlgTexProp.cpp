...
{	
	...
	Viewport->Actor->ShowFlags = SHOW_StandardView | SHOW_ChildWindow | SHOW_RealTime | SHOW_StaticMeshes | SHOW_Actors | SHOW_Frame;
	...
	MaterialsViewport->Actor->ShowFlags = SHOW_StandardView | SHOW_ChildWindow | SHOW_RealTime;
...
void WDlgTexProp::Render_TexView( UViewport* Viewport )
{
	...
	if( Info )
	{
		...
		{
			Viewport->Canvas->DrawIcon( GUnrealEd->MaterialBackdrop, 0, 0, Viewport->SizeX, Viewport->SizeY, 0.0f, FPlane(1,1,1,1));
