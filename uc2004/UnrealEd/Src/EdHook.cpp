		...
		if( ParseObject<UClass>(Item,TEXT("CLASS="),Class,ANY_PACKAGE) )
		{
			...
			for( FObjectIterator It; It; ++It )
			{
				if( It->IsA(Class) && It->GetOuter()!=UObject::GetTransientPackage() )
				{
					...
					{
/*						INT nIndex;
						//scion jg -- Load the entire package here rather than when saving
						// Only load once per refresh so it isn't too slow
						if (List.FindItem(TopParent,nIndex) == 0)
						{
							TopParent->LoadPackage( NULL, TopParent->GetName(),
								LOAD_NoWarn );
						}
...
void UUnrealEdEngine::NotifyExec( void* Src, const TCHAR* Cmd )
{
	...
	else if( ParseCommand(&Cmd,TEXT("NEWOBJECT")) )
	{
		...
		UObject* Parent;
		ParseObject( Cmd, TEXT("PARENT="), Parent, NULL );
		if( Cls && Parent )
		{
			...
			if( Cls->IsChildOf(AActor::StaticClass()) )
			{
				...
				NewObject = StaticConstructObject( Cls, Parent->GetOuter(), NAME_None, RF_Public|((Cls->ClassFlags&CLASS_Localized) ? RF_PerObjectLocalized : 0) );
			if( NewObject )
...
#ifdef WITH_LIPSINC
			case EDC_LIPSincAnimListRtClick:	Msg	= WM_EDC_LIPSINCANIMLISTRTCLICK; break;
#endif
		}
		if( Msg )
		{
