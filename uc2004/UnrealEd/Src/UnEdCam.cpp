...
//
// Freeform movement and rotation on the world X, Y plane -- corrected for view dependency -- or Z axis.
// Works with GUnrealEd->ViewCorrected3DDrag variable in CalcAxialMoveRot below.
//
void CalcViewCorrectedMoveRot
(
	UViewport*		Viewport,
	FLOAT			MouseX,
	FLOAT			MouseY,
...
{
	guard(CalcViewCorrectedMoveRot);
	if( Viewport->IsOrtho() )
	{
	...
	else
	{
		if( (Buttons&(MOUSE_Left|MOUSE_Right))==(MOUSE_Left) )
		{
			...
			FCoords DirectionCoords = GMath.UnitCoords / Viewport->Actor->Rotation;
			FVector Direction = DirectionCoords.XAxis;
			FVector UpVector = DirectionCoords.ZAxis;
			FVector RightVector = DirectionCoords.YAxis;
			FVector FloorAlignedRightVector = GMath.UnitCoords.ZAxis ^ Direction;
			FVector FloorAlignedUpVector = FloorAlignedRightVector ^ GMath.UnitCoords.ZAxis;

			// Ensure we've got legitimate vectors (we won't if the camera is pointing directly up or down).
			if( FloorAlignedRightVector.IsNearlyZero() )
			{
				FloorAlignedRightVector = RightVector;
				FloorAlignedUpVector = UpVector;
			}

			FloorAlignedRightVector.Normalize();
			FloorAlignedUpVector.Normalize();

			Delta		 =  MouseX * FloorAlignedRightVector;
			Delta		+= -MouseY * FloorAlignedUpVector;
		}
		else if( (Buttons&(MOUSE_Left|MOUSE_Right))==(MOUSE_Left|MOUSE_Right) )
		{
			// Both buttons: Move up and down the Z axis.
			Delta.Z		 = -MouseY;
		}
		else if( (Buttons&(MOUSE_Left|MOUSE_Right))==(MOUSE_Right) )
		{
...
)
{
	...
	else
	{
		if( GARGAxis == -1 )
		{
			if( GUnrealEd->ViewCorrected3DDrag )
			{
				CalcViewCorrectedMoveRot( Viewport, MouseX, MouseY, Buttons, Delta, DeltaRot );
			}
			else
			{
				// Do single-axis movement.
...
#ifdef WITH_LIPSINC
	else if( Viewport->Actor->RendMap==REN_LIPSinc )
	{
		return 0;
	}
#endif
...
void DrawViewerBackground( UViewport* Viewport )
{
	...
	Viewport->Canvas->DrawPattern( GUnrealEd->Bkgnd, 0, 0, Viewport->SizeX, Viewport->SizeY, 1.f, 0.f, 0.f, 0.0f, FPlane(1.f,1.f,1.f,1.f));
	unguard;
}
...
void GetMaterialBorderInfo( UMaterial* InMaterial, UBOOL* InDisplayBorder, FColor* InBorderColor, INT* InBorderOffset )
{
	...
	*InBorderColor = FColor(255,255,255);
	...
	if( InMaterial->IsA(UPixelShaderMaterial::StaticClass() ) )
	{
		*InDisplayBorder = 1;
		*InBorderColor = FColor(0,255,255);
	}
	if( InMaterial->IsA(UMultiPassMaterial::StaticClass() ) )
	{
		*InDisplayBorder = 1;
		*InBorderColor = FColor(64,64,255);
	}
...
UBOOL IsMaterialSafe(UMaterial* Material)
{
	guard(IsMaterialSafe);
	UBOOL bSafe = 0;
	if (Material != NULL)
	{
		try
		{
			bSafe = Material->IsValid();
		}
		catch(...)
		{
			bSafe = 0;
...
void DrawTextureBrowser( UViewport* Viewport )
{
	...
	else if( Viewport->Actor->RendMap == REN_TexBrowserUsed )
	{
		...
		for( INT x = 0 ; x < GUnrealEd->Level->Actors.Num() ; ++x )
		{
			...
			if( Actor )
			{
				...
				{
					if( IsMaterialSafe(Actor->Texture) )
					{
						Materials.AddUniqueItem(Actor->Texture);
					}
					else
					{
						debugf(TEXT("Invalid texture for actor %s"),Actor->GetName());
					}
				}
				...
				if( GTBOptions->IUFilter&IUF_Actors && !Cast<AStaticMeshActor>(Actor) )
				{
					...
					{
						if( IsMaterialSafe(Actor->Skins(y))  && (INT)Actor->Skins(y) != 0xFFFFFFFF)
						{
							Materials.AddUniqueItem(Actor->Skins(y));
						}
						else
						{
							debugf(TEXT("Invalid texture for actor %s skin %d"),
								Actor->GetName(),y);
						}
					}
				}
				...
				if( GTBOptions->IUFilter&IUF_Brushes )
				{
					...
					if( Brush && Brush->Brush )
						for( INT y = 0 ; y < Brush->Brush->Polys->Element.Num() ; ++y )
						{
							...
							if( IsMaterialSafe(Poly->Material))
							{
								Materials.AddUniqueItem(Poly->Material);
							}
							else
							{
								debugf(TEXT("Invalid texture for brush %s"),Brush->GetName());
							}
						}
				}
				...
				if( GTBOptions->IUFilter&IUF_StaticMeshes )
				{
					...
					if( SM && SM->StaticMesh )
					{
						for( INT y = 0 ; y < SM->StaticMesh->Materials.Num() ; ++y )
							if( IsMaterialSafe(SM->StaticMesh->Materials(y).Material))
							{
								Materials.AddUniqueItem(SM->StaticMesh->Materials(y).Material);
							}
							else
							{
								debugf(TEXT("Invalid texture for static mesh actor %s with mesh %s"),
									SM->GetName(),SM->StaticMesh->GetName());
							}

						for( INT y = 0 ; y < SM->Skins.Num() ; ++y )
							if( IsMaterialSafe(SM->Skins(y)))
							{
								Materials.AddUniqueItem(SM->Skins(y));
							}
							else
							{
								debugf(TEXT("Invalid skin (%d) for static mesh actor %s"),y,
									SM->GetName());
							}
					}
				}
				...
				if( GTBOptions->IUFilter&IUF_Terrain )
				{
					...
					if( TI )
					{
						...
						{
							if( IsMaterialSafe(TI->Layers[y].AlphaMap) )
							{
								Materials.AddUniqueItem(TI->Layers[y].AlphaMap);
							}
							if( IsMaterialSafe(TI->Layers[y].Texture))
							{
								Materials.AddUniqueItem(TI->Layers[y].Texture);
							}
						}
					}
				}
	...
	else if( Viewport->Actor->RendMap == REN_TexBrowserMRU )
	{
		...
		{
			Materials.AddItem(GTBOptions->MRUMaterials(x));
		}
	}
	...
	{
		appQsort( &Materials(0), Materials.Num(), sizeof(UMaterial*), ResNameCompare );
	}
	...
	*LastScroll = 0;
	if( YL > 0 )
	{
		...
		{
			UMaterial* Material = Materials(i);
			UTexture* Texture = Cast<UTexture>(Materials(i));
			...
			if( Texture )
			{
				TextLabel += *FString::Printf( TEXT("%s [%s]"),
					Texture->Detail ? TEXT("") : TEXT("*"), *Texture->GetFormatDesc());
			}
			else
			{
				FString ErrStr;
				...
				//scion jg -- BSP pass count check
				DWORD StageCount = Viewport->RI->GetStageCountForPass(NumPasses);
				INT BspPasses = NumPasses;
				// Hack to check against xbox texture count limit
				if (StageCount + 1 > 4)
				{
					BspPasses++;
				}
				if( ErrMat )
					...
					TextLabel += *FString::Printf( TEXT(" [%d pass%s / BSP %d pass%s]"), NumPasses, NumPasses==1?TEXT(""):TEXT("es"),BspPasses, BspPasses==1?TEXT(""):TEXT("es") );
			}
			...
			if( Material )
			{
				if (Texture)
				{
					// Show a minimum of 1K
					INT Size = Max<INT>(Texture->MemUsage / 1024,1);
					// Output the size in KB for the texture in addition to width/height
					SizeLabel = FString::Printf( TEXT("(%dx%d) %d KB"),
						Material->MaterialUSize(), Material->MaterialVSize(),
						Size);
				}
				else
				{
			...
			INT TextureWidth = Max<INT>( GTBOptions->GetMaterialWidth( Material ), (LabelWidth > SizeWidth) ? LabelWidth : SizeWidth ),
			...
			if( Y+TextureHeight+LabelHeight+4+BorderOffset>0 && Y<Viewport->SizeY )
			{
				...
				if( bDisplayBorder )
				{
					Viewport->Canvas->DrawPattern( GUnrealEd->Level->GetLevelInfo()->WhiteSquareTexture,
						...
						1.0, 0.0, 0.0, 0.0f, BorderColor.Plane());
					Viewport->Canvas->DrawPattern( GUnrealEd->Level->GetLevelInfo()->WhiteSquareTexture,
						X-4, Y-4,
						TextureWidth+8, TextureHeight+TextBuffer+8,
						1.0, 0.0, 0.0, 0.0f, FColor(0,0,0).Plane());
				}
				...
				if( Material == GUnrealEd->CurrentMaterial )
					Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
						X-4, Y-4,
						TextureWidth+8, TextureHeight+TextBuffer+8,
						1.0, 0.0, 0.0, 0.0f, FPlane(1.,1.,1.,1.));
				...
				Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
					...
					1.0, 0.0, 0.0, 0.0f, ( Material == GUnrealEd->CurrentMaterial ) ? FPlane(1,1,1,1.) : FPlane(0,0,0,1.));
	...
	if( StaticMesh )
	{
		...
		else
		{
			...
			Viewport->Canvas->WrappedPrintf(Viewport->Canvas->SmallFont,1,
 				TEXT("Triangles : %d, Vertices : %d"),StaticMesh->RawTriangles.Num(),
				StaticMesh->VertexStream.Vertices.Num());
#ifdef WITH_KARMA
		}
#endif
	}
...
INT DrawField( UViewport* Viewport, INT InX, INT InY, FColor InLabelColor, FString InLabel, FColor InFieldColor, const TCHAR* Field, ... )
{
	...
	FLOAT Width, Height, TotalWidth = 0;
...
void DrawMaterial( UViewport* InViewport, UMaterial* InMaterial, FString InName, INT InX, INT& InY, UBOOL bEndofChain, UMaterial* CircRefMaterial )
{
	...
	else if( InMaterial->MaterialVSize() > 0 )						Scale = Min<INT>( TREE_ITEM_SZ, InMaterial->MaterialVSize() ) / (FLOAT)InMaterial->MaterialVSize();
	...
	if( (INT)InMaterial == InViewport->Actor->Misc2 )
		InViewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
			...
			1.0, 0.0, 0.0, 0.0f, FColor(255,255,255).Plane());
	else
	...
	else if( Cast<UPixelShaderMaterial>( InMaterial ) )		Title = TEXT("PIXEL SHADER");
	...
	if( InMaterial != CircRefMaterial )
	{
		...
		else if( Cast<UShader>( InMaterial ) )
		{
			...
			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->Bump, TEXT("Bump"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
		...
		else if( Cast<UPSDistortionMaterial>( InMaterial ) )
		{
			UPSDistortionMaterial* Mat = Cast<UPSDistortionMaterial>( InMaterial );

			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->DistortionMaterial, TEXT("DistortionMaterial"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 1 );
			DrawMaterial( InViewport, Mat->Mask, TEXT("Mask"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
		}
		else if( Cast<UPSBlendedDistortionMaterial>( InMaterial ) )
		{
			UPSBlendedDistortionMaterial* Mat = Cast<UPSBlendedDistortionMaterial>( InMaterial );
		...
		else if( Cast<UPixelShaderMaterial>( InMaterial ) )
		{
			//scion jg -- Rendering for each piece of a skin shader
			if( Cast<UPSSkinShader>( InMaterial ) )
			{
				// Get the skin shader pieces
				UPSSkinShader* pSkin = Cast<UPSSkinShader>(InMaterial);
				// Draw the Diffuse
				DrawTreeConnection( InViewport, InX, InY, 0 );
				DrawMaterial( InViewport, pSkin->Diffuse, TEXT("Diffuse + Spec Mask"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
				// Draw the Specular
				DrawTreeConnection( InViewport, InX, InY, 0 );
				DrawMaterial( InViewport, pSkin->Specular, TEXT("Specular"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
				// Check to see if this is a team skin. If it is the rimlight
				// is not the last material
				UPSTeamSkinShader* pTSkin = Cast<UPSTeamSkinShader>(InMaterial);
				// Draw the RimLight
				DrawTreeConnection( InViewport, InX, InY, pTSkin == NULL );
				DrawMaterial( InViewport, pSkin->RimLight, TEXT("Rim Light"), InX+ArrowWidth+BorderOffset, InY, pTSkin == NULL, CircRefMaterial );
				// Check for a team skin
				if(pTSkin != NULL)
				{
					// Draw the TeamMask
					DrawTreeConnection( InViewport, InX, InY, 1 );
					DrawMaterial( InViewport, pTSkin->TeamMask, TEXT("Team Mask"), InX+ArrowWidth+BorderOffset, InY, 1, CircRefMaterial );
				}
			}
			//scion jg -- Rendering for each piece of a combined diffuse
			else if( Cast<UPSDualDiffuse>( InMaterial ) )
			{
				UPSDualDiffuse* pMat = Cast<UPSDualDiffuse>(InMaterial);
				// Draw the Diffuse
				DrawTreeConnection( InViewport, InX, InY, 0 );
				DrawMaterial( InViewport, pMat->Diffuse, TEXT("Diffuse"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
				// Draw the Diffuse
				DrawTreeConnection( InViewport, InX, InY, 0 );
				DrawMaterial( InViewport, pMat->Diffuse2, TEXT("Diffuse 2"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
				// Draw the Specular
				DrawTreeConnection( InViewport, InX, InY, pMat->SpecularMask == NULL );
				DrawMaterial( InViewport, pMat->Specular, TEXT("Specular"), InX+ArrowWidth+BorderOffset, InY, pMat->SpecularMask == NULL, CircRefMaterial );
				// Draw the specular mask if present
				if (pMat->SpecularMask != NULL)
				{
					// Draw the Specular Mask
					DrawTreeConnection( InViewport, InX, InY, 1 );
					DrawMaterial( InViewport, pMat->SpecularMask, TEXT("Specular Mask"), InX+ArrowWidth+BorderOffset, InY, 1, CircRefMaterial );
				}
			}
			//scion jg -- Rendering for each piece of a diffuse/opacity/specular
			else if( Cast<UPSDiffuseOpacitySpecularMask>( InMaterial ) )
			{
				UPSBaseShader* pMat = Cast<UPSBaseShader>(InMaterial);
				// Draw the Diffuse
				DrawTreeConnection( InViewport, InX, InY, 0 );
				DrawMaterial( InViewport, pMat->Diffuse, TEXT("Diffuse/Opacity/SpecMask"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
				// Draw the Specular
				DrawTreeConnection( InViewport, InX, InY, 1 );
				DrawMaterial( InViewport, pMat->Specular, TEXT("Specular"), InX+ArrowWidth+BorderOffset, InY, 1, CircRefMaterial );
			}
			//scion jg -- Rendering for each piece of a emissive shader
			else if( Cast<UPSEmissiveShader>( InMaterial ) )
			{
				UPSEmissiveShader* pMat = Cast<UPSEmissiveShader>(InMaterial);
				// Draw the Diffuse
				DrawTreeConnection( InViewport, InX, InY, 0 );
				DrawMaterial( InViewport, pMat->Diffuse, TEXT("Diffuse"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
				if (pMat->Specular != NULL)
				{
					// Draw the Specular
					DrawTreeConnection( InViewport, InX, InY, 0 );
					DrawMaterial( InViewport, pMat->Specular, TEXT("Specular"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
					if (pMat->SpecularMask != NULL)
					{
						// Draw the Specular mask
						DrawTreeConnection( InViewport, InX, InY, 0 );
						DrawMaterial( InViewport, pMat->SpecularMask, TEXT("Specular Mask"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
					}
				}
				// Draw emissive mask
				DrawTreeConnection( InViewport, InX, InY, 1 );
				DrawMaterial( InViewport, pMat->EmissiveMask, TEXT("Emissive Mask"), InX+ArrowWidth+BorderOffset, InY, 1, CircRefMaterial );
			}
			//scion jg -- Rendering for each piece of a team shader
			else if( Cast<UPSTeamShader>( InMaterial ) )
			{
				UPSTeamShader* pMat = Cast<UPSTeamShader>(InMaterial);
				// Draw the Diffuse
				DrawTreeConnection( InViewport, InX, InY, 0 );
				DrawMaterial( InViewport, pMat->Diffuse, TEXT("Diffuse"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
				// Draw the Specular
				...
				// Draw emissive mask
				DrawTreeConnection( InViewport, InX, InY, 1 );
				DrawMaterial( InViewport, pMat->TeamMask, TEXT("Team Mask"), InX+ArrowWidth+BorderOffset, InY, 1, CircRefMaterial );
			}
		}
...
void DrawTerrainHeightmap( UViewport* Viewport )
{
	...
	if( TerrainInfo )
	{
		...
		Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
			...
			1.0, 0.0, 0.0, 0.0, BackgroundColor.Plane());
...
void DrawLayer( UViewport* Viewport, ATerrainInfo* InTerrainInfo, INT InLayer, INT CurX, INT CurY )
{
	...
	if( !bPaintingAllowed )
		Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
			...
			1.0, 0.0, 0.0, 0.0, FColor(128,0,0).Plane());
	...
	if( InTerrainInfo->Layers[InLayer].AlphaMap )
	{
		...
		Viewport->Canvas->CurX = CurX+LAYER_SZ+LAYER_SZ+8;
		Viewport->Canvas->WrappedPrintf( Viewport->Canvas->SmallFont, 0,
			*FString::Printf( TEXT("Scale : %1.1fx%1.1f"), InTerrainInfo->Layers[InLayer].UScale, InTerrainInfo->Layers[InLayer].VScale ) );
	}
	else
	{
...
void DrawDecoLayer( UViewport* Viewport, ATerrainInfo* InTerrainInfo, INT InLayer, INT CurX, INT& CurY )
{
	...
	Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
		...
		1.0, 0.0, 0.0, 1.0, BackgroundColor.Plane());
...
void DrawActionBar( UViewport* Viewport, INT InCurX, INT InCurY, ASceneManager* SM )
{
	...
	for( INT ma = 0 ; ma < SM->Actions.Num() ; ++ma )
	{
		...
		Viewport->Canvas->DrawPattern( White,
			InCurX + Start, InCurY,
			Size, 4,
			1.0, 0.0, 0.0, 0.0f, Color.Plane());

		Viewport->Canvas->DrawPattern( White,
			InCurX + Start, InCurY-1,
			1, 6,
			1.0, 0.0, 0.0, 0.0f, FColor(255,255,0).Plane());
	...
	for( INT x = 0 ; x < Viewport->Actor->GetLevel()->Actors.Num() ; ++x )
	{
		...
		if( SM )
		{
			...
			Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
				...
				1.0, 0.0, 0.0, 0.0f, FPlane(.75,.75,.75,1));

				Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
					CurX, CurY,
					Viewport->SizeX-6, FMatineeTools::MAT_SCENE_HEIGHT,
					1.0, 0.0, 0.0, 0.0f, GMatineeTools.GetCurrent() == SM ? FPlane(1.,1.,1.,1) : FPlane(0,0,0,1));
	...
	for( INT x = 0 ; x < SM->Actions.Num() ; ++x )
	{
		...
		Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
			...
			Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
				...
				1.0, 0.0, 0.0, 0.0f, GMatineeTools.GetCurrentAction() == Action ? FPlane(1.,1.,1.,1) : FPlane(0,0,0,1));
		...
		Viewport->Canvas->DrawPattern( White,
			CurX, CurY,
			Viewport->SizeX-6, 4,
			1.0, 0.0, 0.0, 0.0f, FPlane(.5f,.5f,.5f,1));
		...
		Viewport->Canvas->DrawPattern( White,
			...
			Sz, 4,
			1.0, 0.0, 0.0, 0.0f, FPlane(1.f,1.f,1.f,1));
	...
	for( INT x = 0 ; x < MA->SubActions.Num() ; ++x )
	{
		...
		Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
			...
			Viewport->Canvas->DrawPattern( GUnrealEd->BkgndHi,
				...
				1.0, 0.0, 0.0, 0.0f, GMatineeTools.GetCurrentSubAction() == SubAction ? FPlane(1.,1.,1.,1) : FPlane(0,0,0,1));

		// ICON
		//scion jg -- Fixed the crash if there is no icon bug
		if (SubAction->Icon)
		{
		...
		{
			CurY += FontHeight + FontHeight;
			DrawField( Viewport, CurX, CurY, FColor(192,192,192), SubAction->GetStatString(), FColor(0,255,255),TEXT(""));
			CurY += FontHeight;
		}
		...
		if( Cast<USubActionOrientation>(SubAction) )
		{
			...
			Viewport->Canvas->DrawPattern( White,
				...
				Sz, 4,
				1.0, 0.0, 0.0, 0.0f, FPlane(0.75f,0.75f,.75f,1));
		}
...
//
void UUnrealEdEngine::Draw( UViewport* Viewport, FLOAT DeltaSeconds, UBOOL Blit, BYTE* HitData, INT* HitCount )
{
...
#ifdef WITH_LIPSINC
	if( Actor->RendMap==REN_MeshView || Actor->RendMap==REN_Animation || Actor->RendMap==REN_LIPSinc )
#else
		...
		switch( Actor->RendMap )
		{
			...
			case REN_TexView:
			{
				...
				else
				{
					...
					if( Material )
						Viewport->Canvas->DrawIcon( Material, 0, 0, Viewport->SizeX, Viewport->SizeY, 0.0f, FPlane(1,1,1,1));
				}
				break;
			}
			default:
...
)
{
	...
	if( !GEdModeTools->HasMouseMoved() )
	{
		...
		Draw( Viewport, 0.f, 0, HitData, &HitCount );
...
#ifdef WITH_LIPSINC
		case REN_LIPSinc:				return EM_LIPSinc;
#endif
...
//	======================================================================================
//	UEditorEngine::Bump
//	input:    current Viewport, ph +1/0/-1 to indicate left/right, pv same for vertical
//	output:   
//	effects:  
//	notes:    moves currently selected item around Ortho window
//			 called from UnrealEd/ViewportFrame.h
//	last modified by:  capps
//	======================================================================================
*/
void UUnrealEdEngine::Bump(UViewport* Viewport, INT ph, INT pv){
	if (!(ph == 0 || ph == 1 || ph == -1)){
		appMsgf( 0, TEXT("Bump called with illegal arguments."));
		return;	
	}
	if (!(pv == 0 || pv == 1 || pv == -1)){
		appMsgf( 0, TEXT("Bump called with illegal arguments."));
	...
	if (!Viewport->IsOrtho()){
		//appMsgf(TEXT("Bump only works on an ortho viewport."));
		return;
	}
	...
	switch (Viewport->Actor->RendMap) {
		case REN_OrthXZ:
			delta.X += ph*Constraints.GridSize.X;
			delta.Z += pv*Constraints.GridSize.Z;
			break;
		case REN_OrthXY:
			delta.X += ph*Constraints.GridSize.X;
			delta.Y -= pv*Constraints.GridSize.Y;
			break;
		case REN_OrthYZ:
			delta.Y += ph*Constraints.GridSize.Y;
			delta.Z += pv*Constraints.GridSize.Z;
		break;
	}
	...
	case EM_ActorSnapScale:{
		...
		for( INT i = 0 ; i < Level->Actors.Num() ; i++ )
		{
//          In case we want to try supporting bumping for actors:
//			AActor* Actor = Level->Actors(i);
//			if( Actor && Actor->bSelected )
//				BBox += Actor->GetPrimitive()->GetRenderBoundingBox( Actor );
			ABrush* Brush = Cast<ABrush>(Level->Actors(i));
			if( Brush && Brush->bSelected )
				BBox += Brush->GetPrimitive()->GetRenderBoundingBox( Brush );
		}
	...
	// if in vertex edit mode, bump around the selected vertices
		//todo capps Make UNDO work on these, but note, it doesn't work for regular vertex edit 
		if ( VertexHitList.Num() > 0){  
			for (INT vertex = 0; vertex < VertexHitList.Num(); vertex++){
				FVector* Vtx = &(VertexHitList(vertex).pBrush->Brush->Polys->Element(VertexHitList(vertex).PolyIndex).Vertex[VertexHitList(vertex).VertexIndex]);
				FVector FVertex = Vtx->TransformPointBy( VertexHitList(vertex).pBrush->ToWorld() );
				//debugf(NAME_Log, TEXT("Vertex is originally at [%f,%f,%f]"),FVertex.X, FVertex.Y, FVertex.Z);
				FVertex += delta;
				*Vtx = FVertex.TransformPointBy( VertexHitList(vertex).pBrush->ToLocal() );
				//debugf(NAME_Log, TEXT("Vertex is now at [%f,%f,%f]"),Vtx->X, Vtx->Y, Vtx->Z);
			}
			break;
		}
	...
	// else falls through
	default:
		// otherwise, move any selected actors
		FRotator rot(0,0,0);

		// get selected actor and bump it
		for( INT i=0; i<Level->Actors.Num(); i++ ) {
			AActor* Actor = Level->Actors(i);
			if( Actor && Actor->bSelected ){
				MoveSingleActor( Actor, delta, rot);
				// used to have different code for brush:
				// if (Actor->IsBrush()){    
				//Actor->Location.AddBounded( delta, HALF_WORLD_MAX1 );
			}
		}  // end for
	...
	//Viewport->Repaint(1);  // this is the cheap way, just the ortho view updates
	EdCallback( EDC_RedrawAllViewports, 0, 0 );  // this is more expensive, but why not?
}
