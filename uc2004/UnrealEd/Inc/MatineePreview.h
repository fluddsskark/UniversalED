...
class WMatineePreview : public WWindow
{
	...
	void OnResetClick()
	{
		...
		if( SM->Affect == AFFECT_ViewportCamera )
		{
			...
			if( PCSrc && PCDst && PCSrc->ScreenFlash && PCDst->ScreenFlash)
			{
				UScreenFlash* pSrc = Cast<UScreenFlash>(PCSrc->ScreenFlash);
				UScreenFlash* pDest = Cast<UScreenFlash>(PCDst->ScreenFlash);
				check(pSrc && pDest);
				pDest->FlashFog = pSrc->FlashFog;
				pDest->FlashScale = pSrc->FlashScale;
