...
class WDlgImportTexture : public WDialog
{
	...
	WCheckBox DistortionMapCheck;
	WComboBox FormatCombo;
	WComboBox BumpmapCombo;
	...
	,   DistortionMapCheck( this, IDCK_DISTORTION_MAP )
	,	FormatCombo		( this, IDCB_TEXTUREFORMAT )
	,	BumpmapCombo	( this, IDCB_BUMPMAPPING )
	{
	}
	...
	void OnInitDialog()
	{
		...
		FormatCombo.AddString( TEXT("RGBA8") );
		FormatCombo.AddString( TEXT("DXT1") );
		FormatCombo.AddString( TEXT("DXT3") );
		FormatCombo.AddString( TEXT("DXT5") );
		FormatCombo.AddString( TEXT("A8") );
		FormatCombo.AddString( TEXT("L8") );
		FormatCombo.SetCurrent(0);

		BumpmapCombo.AddString( TEXT("None") );
		BumpmapCombo.AddString( TEXT("16bit [V8U8]") );
		BumpmapCombo.AddString( TEXT("16bit+Alpha [L6V5U5]") );
		BumpmapCombo.AddString( TEXT("32bit [V16U16]") );
		BumpmapCombo.SetCurrent(0);
	...
	void ImportFile( FString Filename )
	{
		...
		{
			GUnrealEd->Exec( *FString::Printf(TEXT("TEXTURE DETAILHACK NAME=%s FORMAT=%s"),
				*TextureName, *FormatCombo.GetString( FormatCombo.GetCurrent() )));
		}

		if( BumpmapCombo.GetCurrent() > 0 )
		{
			TCHAR* texFormat[4] = { TEXT("NONE"), TEXT("TEXF_V8U8"), TEXT("TEXF_L6V5U5"), TEXT("TEXF_V16U16") };
			GUnrealEd->Exec( *FString::Printf( TEXT("TEXTURE BUMPMAPGEN NAME=%s FORMAT=%s"), *TextureName, texFormat[BumpmapCombo.GetCurrent()] ) );
		}

		if( DistortionMapCheck.IsChecked() )
		{
			GUnrealEd->Exec( *FString::Printf( TEXT("TEXTURE DISTORTIONMAPGEN NAME=%s"), *TextureName) );
		}
		
		if( FormatCombo.GetCurrent() > 0 )
		{
			GUnrealEd->Exec( *FString::Printf(TEXT("TEXTURE CONVERT NAME=%s FORMAT=%s"),
				*TextureName, *FormatCombo.GetString( FormatCombo.GetCurrent() ) ) );
		}
...
class WBrowserTexture : public WBrowser
{
	...
	void OnCommand( INT Command )
	{
		...
		switch( Command )
		{
			...
			case IDMN_TB_COMPRESS_A8:
			case IDMN_TB_COMPRESS_L8:
			{
				...
				if( CurrentTexture )
				{
					...
					else if( Command == IDMN_TB_COMPRESS_A8 ) Format = TEXT("A8");
					else if( Command == IDMN_TB_COMPRESS_L8 ) Format = TEXT("L8");
			...
			case IDMN_TB_DELETE:
			{
				...
				FString Name = GUnrealEd->CurrentMaterial->GetName();
				FString PathName = GUnrealEd->CurrentMaterial->GetPathName();
				FStringOutputDevice GetPropResult = FStringOutputDevice();
				TCHAR l_chCmd[256];

				if( ::MessageBox( hWnd, TEXT("Deleting this material will replace all references to this material. Are you sure you want to delete?"), TEXT("Delete Material"), MB_YESNO) == IDNO)
				{
					break;
				}
				...
				//jg -- Use the special archive to sever links to this material
				for (TObjectIterator<UObject> It; It; ++It)
				{
					// Don't ask it to clean up itself
					if (*It != Save)
					{
						// Ask this object to clean up any references
						FArchiveReplaceObjectRef ReplaceRef(*It,Save);
						if (ReplaceRef.GetCount() > 0)
						{
							debugf(TEXT("Material %s referenced %s"),It->GetName(),Save->GetName());
						}
					}
				}

				// scion sz -
				// if the texture name has a "." (this is an error)
				// then just delete the object without using an exec
				if( Name.InStr(TEXT(".")) != INDEX_NONE && 
					Save )
				{
					guard(ForceRemoveMaterial);
					UObject* Object = GUnrealEd->CurrentMaterial; 
					UObject* Pkg = Object;
					while( Pkg->GetOuter() )
						Pkg = Pkg->GetOuter();
			...
			case IDMN_TB_EXPORT_PCX:
			{
				...
				case TEXF_DXT3:
				case TEXF_DXT5:
					ofn.lpstrFilter = "DDS (*.dds)\0*.dds";
					ofn.lpstrDefExt = "dds";
					break;
				default:
					ofn.lpstrFilter = "Can't export this file type!\0*.unk";
					ofn.lpstrDefExt = "unk";
					break;
				}
			...
			//scion jg -- Allows setting the material type for an entire package
			case IDM_TEXTURE_SETMATERIAL:
			{
				// Work up the outer tree to find the containing package
				UPackage* pPackage =
					(UPackage*)GUnrealEd->CurrentMaterial->GetOuter();
				// If this material is in a group, it will need to search again
				while (Cast<UPackage>(pPackage) == NULL)
				{
					pPackage = (UPackage*)pPackage->GetOuter();
				}
				// Now that we have the package, iterate through setting the type
				for (TObjectIterator<UMaterial> It; It; ++It)
				{
					// If it is in the matching package
					if (It->IsIn(pPackage))
					{
						// Set its type to the currently selected material
						It->MaterialType = GUnrealEd->CurrentMaterial->MaterialType;
					}
				}
			}
			break;

			//scion jg -- Allows setting the LODSet for an entire package
			case IDM_TEXTURE_SETLODSET:
			{
				// Make sure the currently selected material is the right type
				if (GUnrealEd->CurrentMaterial->IsA(UTexture::StaticClass()))
				{
					// Work up the outer tree to find the containing package
					UPackage* pPackage = CastChecked<UPackage>(
						GUnrealEd->CurrentMaterial->GetOuter());
					// If this material is in a group, it will need to search again
					while (pPackage != NULL && pPackage->GetOuter())
					{
						pPackage = CastChecked<UPackage>(pPackage->GetOuter());
					}
					// Now that we have the package, iterate through setting the LOD
					for (TObjectIterator<UMaterial> It; It; ++It)
					{
						// If it is in the matching package and is a UTexture
						if (It->IsIn(pPackage) && It->IsA(UTexture::StaticClass()))
						{
							// Set its type to the currently selected material
							((UTexture*)*It)->LODSet =
								((UTexture*)GUnrealEd->CurrentMaterial)->LODSet;
						}
					}
				}
			}
			...
			//jg -- Removes mips from a given texture
			case IDM_TEXTURE_REMOVEMIPS:
			{
				// Make sure the currently selected material is the right type
				if (GUnrealEd->CurrentMaterial->IsA(UTexture::StaticClass()))
				{
					UTexture* pTexture = (UTexture*)GUnrealEd->CurrentMaterial;
					if (pTexture->Mips.Num() > 1)
					{
						// Rebuild mips
						pTexture->CreateMips(0,0);
						// Mark the package as dirty
						UObject* Pkg = pTexture;
						while( Pkg->GetOuter() )
						{
							Pkg = Pkg->GetOuter();
						}
			...
			//jg -- Removes mips from a given texture
			case IDM_TEXTURE_PACK_REMOVEMIPS:
			{
				// Make sure the currently selected material is the right type
				if (GUnrealEd->CurrentMaterial->IsA(UTexture::StaticClass()))
				{
					...
					// Now that we have the package, iterate through removing mips
					for (TObjectIterator<UMaterial> It; It; ++It)
					{
						// If it is in the matching package and is a UTexture
						if (It->IsIn(pPackage) && It->IsA(UTexture::StaticClass()))
						{
							UTexture* pTexture = (UTexture*)*It;
							// Rebuild mips
							pTexture->CreateMips(0,0);
						}
					}
