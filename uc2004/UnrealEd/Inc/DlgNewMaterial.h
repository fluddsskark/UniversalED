...
class WDlgNewMaterial : public WDialog
{
		...
		if( Factory )
		{
			...
			for( TFieldIterator<UProperty> It(Factory->GetClass()); It; ++It )
				if( It->Category==FName(Factory->GetClass()->GetName()) && pProps->Root.AcceptFlags( It->PropertyFlags ) )
					pProps->Root.Children.AddItem( NEW FPropertyItem( pProps, &(pProps->Root), *It, It->GetFName(), It->Offset, -1, 666 ) );
			pProps->Root.Expand();
			pProps->ResizeList();
	...
	BOOL GetDataFromUser()
	{
		...
		if( !Package.Len() || !Name.Len() || 
			// scion sz - make sure texture name does not have a "."
			Name.InStr(TEXT(".")) != INDEX_NONE )
		{
