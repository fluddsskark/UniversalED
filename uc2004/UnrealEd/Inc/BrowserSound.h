...
#if 1	//scion rwd - restore the selection to the renamed sound
						// refresh the packages
						RefreshPackages();
						// and select the proper package
						pComboPackage->SetCurrent(pComboPackage->FindStringExact(*dlg.NewPackage));
						// refresh the groups
						RefreshGroups();
						// and select the proper group
						pComboGroup->SetCurrent(pComboGroup->FindStringExact(*dlg.NewGroup));
						// refresh the sound list
						RefreshSoundList();
						// and select the proper sound
						pListSounds->SetCurrent(pListSounds->FindStringExact(*dlg.NewName));
#else
						RefreshAll();
#endif
					}
