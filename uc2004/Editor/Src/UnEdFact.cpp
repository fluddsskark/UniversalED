...
------------------------------------------------------------------------------*/
// ----- Static data members
// directory is relative to System
const TCHAR * const UClassFactoryUC::processedFileDirectory = TEXT("PreProcessedFiles/");
const TCHAR * const UClassFactoryUC::processedFileExtension = TEXT(".PUC");
const TCHAR * const UClassFactoryUC::processedFileOutputParameterName = TEXT("intermediate");
const TCHAR * const UClassFactoryUC::suppressPreprocessorParameterName = TEXT("nopreprocess");
...
)
{
	...
	//scion bcl - added to preprocess input
	FString ProcessedBuffer;
	if (!ParseParam(appCmdLine(), suppressPreprocessorParameterName)) {

		// filter for stripping comments from running text
		FString sourceFileName = *Name;
		sourceFileName = sourceFileName + TEXT(".") + Type;
		CommentStrippingFilter CommentStripper(sourceFileName);
		MacroProcessingFilter Macro(InParent->GetName(), sourceFileName);
		SequencedTextFilter Filter(CommentStripper, Macro, sourceFileName);
		Filter.Process(Buffer, BufferEnd, ProcessedBuffer);

		if (ParseParam(appCmdLine(), processedFileOutputParameterName)) {
			FString iFileName = *Name;
			FString iDirName = processedFileDirectory;
	...
	// Original content of FactoryCreateText
	...
	for( INT i=0; i<InLevel->Actors.Num(); i++ )
	{
		...
		if( Actor )
		{
			...
			if( PropText )
			{
				...
				//scion jg -- Remove the actor in case its collision
				// parameters change
				if (Actor->IsInOctree() && InLevel->Hash)
				{
					Actor->bCollideActors = 1;
					InLevel->Hash->RemoveActor(Actor);
				}
				ImportProperties( Actor->GetClass(), (BYTE*)Actor, InLevel, **PropText, InParent, Warn, 0 );
				//scion jg -- Now re-add the actor if its collision is on
				// post import
				if (Actor->bCollideActors && InLevel->Hash)
				{
					InLevel->Hash->AddActor(Actor);
				}
				ActorChanged = 1;
				unguard;
			}
...
)
{
	...
	while( ParseLine( &Buffer, StrLine ) )
	{
		...
		// scion sz - 
		// import light map scale for this poly surface
		else if( ParseCommand(&Str,TEXT("LIGHTMAPSCALE")) )
		{
			Poly.LightMapScale = appAtof( *ParseToken(Str,0) );
		}
...
)
{
	...
	(	appStricmp(FileType, TEXT("T3D"))==0 )
	{
		...
		Prefab->T3DText = (ANSICHAR*)Buffer;
		return Prefab;
	}
	else
	{
...
UBOOL UTextureExporterTGA::ExportBinary( UObject* Object, const TCHAR* Type, FArchive& Ar, FFeedbackContext* Warn )
{
	...
	{
		Texture->Mips(0).DataArray.Load();
		if (Texture->Mips(0).DataArray.Num() == 0)
		{
			debugf(TEXT("Exporting %s as solid white because of corrupt mips"),Texture->GetName());
		}
		...
		{
			// Don't crash if the mips are invalid
			if (Texture->Mips(0).DataArray.Num() > 0)
			{
			...
			{
				// Write out white for this texture
				for (INT Count = 0; Count < Texture->USize; Count++)
				{
					DWORD Value = 0xFFFFFFFF;
					Ar.Serialize(&Value,4);
				}
			}
		}
