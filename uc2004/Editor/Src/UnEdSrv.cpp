...
#if 0 //scion rwd - disable resetloaders, script optimization (see unprog #30187)
			if( *PackageName )
		...
		if( ParseCommand(&Str,TEXT("Import")) )
		{
			...
			if( PkgName!=NAME_None && Parse( Str, TEXT("File="), TempFname, ARRAY_COUNT(TempFname) ) )
			{
				...
				if( Texture )
				{
					...
					BYTE Compression;
					if ( Parse( Str, TEXT("COMPRESSION="), Compression ) )
					{
						if ( Compression != TEXF_RGBA8 )
							Texture->ConvertFormat(ETextureFormat(Compression));
					}
...
UBOOL UEditorEngine::Exec_StaticMesh( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Str,TEXT("IMPORT")) )
	{
		...
		if( PkgName != NAME_None && Parse( Str, TEXT("FILE="), TempFname, ARRAY_COUNT(TempFname) ) )
		{
			...
			if (appStrstr(TempFname,TEXT(".lwo")) || appStrstr(TempFname,TEXT(".LWO")))
			{
				extern EDITOR_API UStaticMesh *CreateStaticMeshFromLWO(UObject* InOuter, FName InName, const FString &InFileName, ULevel *Level, UClass* Class);
				StaticMesh = CreateStaticMeshFromLWO(Pkg,FName(TempName),FString(TempFname),GEditor->Level,AStaticMeshActor::StaticClass());
			}
			else
			{
				StaticMesh = ImportObject<UStaticMesh>( GEditor->Level, Pkg, TempName, RF_Public|RF_Standalone, TempFname );
			}
			if( StaticMesh )
			{
...
UBOOL UEditorEngine::Exec_Map( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Str,TEXT("REBUILD")) )
	{
		...
		GWarn->StatusUpdatef( 0, 0, TEXT("Flushing...") );
		Flush(0);

		GWarn->StatusUpdatef( 0, 0, TEXT("Redraw level...") );
		RedrawLevel( Level );

		GWarn->StatusUpdatef( 0, 0, TEXT("Map change...") );
	...
	// scion sz - rebuilds everything (uses the GRebuildTools options as specified in the unreal editor)
	//
	else if( ParseCommand(&Str,TEXT("REBUILDALL")) )
	{
		if( GRebuildTools.GetCurrent()->Options & REBUILD_Geometry )
			Exec( TEXT("MAP REBUILD") );

		if( GRebuildTools.GetCurrent()->Options & REBUILD_BSP )
			Exec( TEXT("BSP REBUILD") );

		if( GRebuildTools.GetCurrent()->Options & REBUILD_Lighting )
			Exec( TEXT("LIGHT APPLY") );

		if( GRebuildTools.GetCurrent()->Options & REBUILD_Paths )
			Exec( TEXT("PATHS DEFINE") );

		if( GRebuildTools.GetCurrent()->Options & REBUILD_Fluids )
			Exec( TEXT("FLUID REBUILD") );

		// don't display map check dialog
		GWarn->MapCheck_Hide();
	...
	else if (ParseCommand (&Str,TEXT("SCALE")))
	{
		...
		for( INT i=0; i<GEditor->Level->Actors.Num(); i++ )
		{
			...
			if( Actor && Actor->bSelected )
			{
				...
				else if( Actor->DrawType == DT_StaticMesh || Actor->IsDTMesh() )
				{
	...
	else if (ParseCommand (&Str,TEXT("MEMSTAT")))
	{
		FEdLevelStat levelStats;
		
		Ar.Logf( NAME_Log, TEXT("[MEMSTAT] Analyzing map: '%s'"), Level->GetFullName() );
		Ar.Logf( NAME_Log, TEXT("============================================================") );
		
		levelStats.Analyze( Level );

		if( ParseCommand(&Str,TEXT("DETAIL")) )
		{
			levelStats.PrintTextureDetails( Ar );
		}

		levelStats.PrintTextureSummary( Ar );		
		levelStats.PrintGeomSummary( Ar );

		Ar.Logf( NAME_Log, TEXT("============================================================") );
	}
	// scion sz - scale all lights by factor (only affects brightness)
	else if (ParseCommand (&Str,TEXT("SCALELIGHTS")))
	{
		FLOAT Factor = 1;
		if( !Parse(Str,TEXT("FACTOR="),Factor) ) return 0;

		GWarn->BeginSlowTask( TEXT("Light Scaling"), 1);
...
UBOOL UEditorEngine::Exec_Texture( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if(ParseCommand(&Str,TEXT("DETAILHACK")))
	{
		...
		if(ParseObject<UTexture>(Str,TEXT("NAME="),Texture,ANY_PACKAGE))
		{
			...
			//jg -- Figure out if we are compressing this detail texture or not
			FString FormatStr;
			ETextureFormat NewFormat = TEXF_RGBA8;
			if( Parse( Str, TEXT("FORMAT="), FormatStr ) )
			{
				if( FormatStr == TEXT("DXT1") )
					NewFormat = TEXF_DXT1;
				else if( FormatStr == TEXT("DXT3") )
					NewFormat = TEXF_DXT3;
				else if( FormatStr == TEXT("DXT5") )
					NewFormat = TEXF_DXT5;
				else
					NewFormat = TEXF_MAX;
			}

			// Only generate the gray if this isn't being compressed
			if (Texture->Format == TEXF_RGBA8 && NewFormat == TEXF_RGBA8)
			{
			...
			//jg -- Allow for detail textures to be compressed
			else if (Texture->Format == TEXF_RGBA8 &&
				(NewFormat == TEXF_DXT1 || NewFormat == TEXF_DXT3 ||
				NewFormat == TEXF_DXT5))
			{
				FDXTCompressionOptions Options;
				appMemzero(&Options,sizeof(FDXTCompressionOptions));
				// Tell the compressor that we want detail fading
				Options.Fade = 1;
				Options.FadeToColor = FColor(127,127,127);
				Options.FadeAmount = 50;
				// Now create the compressed fading mips
				Texture->Compress(NewFormat,1,&Options);
			}
			else
			{
	...
	// scion sz -
	else if( ParseCommand(&Str,TEXT("CONVERT")) )
	{
		guard(Exec_Texture CONVERT);
		...
		if( Parse( Str, TEXT("FORMAT="), FormatStr ) )
		{
			ETextureFormat Format = TEXF_RGB8;
			...
			if( FormatStr == TEXT("DXT1") )
				...
				Format = TEXF_DXT5;
			else if( FormatStr == TEXT("A8") )
				Format = TEXF_A8;
			else if( FormatStr == TEXT("L8") )
				Format = TEXF_L8;
			else
				Ar.Logf(TEXT("Unknown texture."));
			...
			case TEXF_DXT5:
				{
					...
					else if(ParseObject<UTexture>(Str,TEXT("NAME="),Texture,ANY_PACKAGE))
					{
						if( !Texture->Compress( Format, 1 ) )
							Ar.Logf(TEXT("Compression on texture '%s' failed."), Texture->GetPathName());
					}
			...
			default:
				{
					if(ParseObject<UCubemap>(Str,TEXT("NAME="),Cubemap,ANY_PACKAGE))
					{
						for( INT i=0; i<6; i++ )
						{
							...
							if( Texture )
							{
								if( !Texture->ConvertFormat( Format ) )
									Ar.Logf(TEXT("Conversion on texture '%s' failed."), Texture->GetPathName());
							}
							else
								Ar.Logf(TEXT("Missing face in cubemap '%s'."), Cubemap->GetPathName());
						}
	...
	else if(ParseCommand(&Str,TEXT("BUMPMAPGEN")))
	{
		//
		// Generate dU,dV values from a heightfield texture to represent a bump map
		//
		//
		UTexture* Texture;
		if( ParseObject<UTexture>(Str,TEXT("NAME="),Texture,ANY_PACKAGE) )
		{
			...
			if( Parse( Str, TEXT("FORMAT="), FormatStr ) )
			{
				ETextureFormat Format = TEXF_V8U8;
				if( FormatStr == TEXT("TEXF_V8U8") )		Format = TEXF_V8U8;
				else if( FormatStr == TEXT("TEXF_L6V5U5") )	Format = TEXF_L6V5U5;
				else if( FormatStr == TEXT("TEXF_V16U16") )	Format = TEXF_V16U16;

				Ar.Logf( NAME_Progress, TEXT("Generating bump map from heightfield texture") );
				Texture->GenerateBumpmap( Format );
			}
		}

		return 1;
	}
	else if( ParseCommand(&Str,TEXT("DISTORTIONMAPGEN")) )
	{
		// create a distortion (V8U8) texture similar to BUMPMAPGEN except input texture
		// is simply treated as grayscale distortion amounts and not as a heightfield
		UTexture* Texture;
		if( ParseObject<UTexture>(Str,TEXT("NAME="),Texture,ANY_PACKAGE) )
		{
			FString FormatStr;
//			ETextureFormat Format = TEXF_V8U8;
			ETextureFormat Format = TEXF_RGBA8;
			Ar.Logf( NAME_Progress, TEXT("Generating distortion map from texture") );
			Texture->GenerateDistortionMap( Format );
		}
	}
...
#if 1//scion jg -- Check for a mylevel reference outside of the package
			if (appStricmp(Pkg->GetName(),TEXT("mylevel")))
			{
				// Construct our DefProps exporter
				UExporter* pExporter = ConstructObject<UExporter>(
					UObjectExporterDefProps::StaticClass());
				// Iterate all objects, check the package they are in,
				// and check each object for a mylevel reference
				for (TObjectIterator<UObject> It; It; ++It)
				{
					if (It->IsIn(Pkg))
					{
						FStringOutputDevice Ar;
						// Export the object to text and search for MyLevel
						UExporter::ExportToOutputDevice(*It,pExporter,
							Ar,TEXT("copy"),0);
						if (Ar.InStr(TEXT("MyLevel")) != -1 ||
							Ar.InStr(TEXT("myLevel")) != -1 )
						{
							appMsgf(0,TEXT("Failed to save due to a MyLevel reference"));
							return 0;
						}
					}
				}
...
#if 0 //scion jg -- Be gone evil code
...
UBOOL UEditorEngine::Exec_Audio( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	//scion rwd - quick and dirty downsample of audio
	else
	if (ParseCommand(&Str,TEXT("DOWNSAMPLE")))
	{
		// iterate through all sound objects
		for (TObjectIterator<USound> It; It != NULL; ++It)
		{
			// get a wave info for the data
			FWaveModInfo WaveInfo;
			It->Data.Load();
			if (WaveInfo.ReadWaveInfo(It->Data))
			{
				// if above 22.5, halve the sample rate
				if (*WaveInfo.pSamplesPerSec > 22050)
				{
					debugf(TEXT("down sampling sound %s"),It->GetFullName());
					WaveInfo.HalveData();
					// and update the sound date
					WaveInfo.UpdateWaveData(It->Data);
				}
			}
		}
		return 1;
...
UBOOL UEditorEngine::Exec( const TCHAR* Stream, FOutputDevice& Ar )
{
	...
	else if ( ParseCommand(&Str,TEXT("ENGINE")) )
	{
		if ( Super::Exec(Str, Ar) )
			return 1;
	}
	//------------------------------------------------------------------------------------
	// MISC
	...
	// MAPS
	//
	// scion sz - runs an exec command on a set of maps
	// Opens each map. Runs the exec command. Saves the map.
	//
	// Usage:
	// MAPS <Wildcard> <Command>
	//
	// Example:
	// MAPS ../Maps/*.unr "LIGHT APPLY"
	//
	else if (ParseCommand (&Str,TEXT("MAPS")))
	{
		FString WildCard, Command;

		// Make sure we got all params.
		if( ParseToken(Str,WildCard,0) )
		{
			...
			if( ParseToken(Str,Command,0) )
			{
				if( FilesFound.Num() )
				{
					for (INT i=0; i<FilesFound.Num(); i++)
					{
						...
						{
							Exec( *FString::Printf(TEXT("MAP LOAD FILE=\"%s%s\""), *Dir, *Pkg), Ar );
							UObject* Package = CastChecked<UPackage>(GEditor->Level->GetOuter());
							if (Package != NULL)
							{
								// Execute command on the map
								Ar.Logf( TEXT("Executing command [%s] on map [%s]"), *Command, *Pkg );
								GEditor->Exec( *Command, Ar );

								// Save the updated map
								GEditor->Exec( *FString::Printf(TEXT("MAP SAVE FILE=\"%s%s\""), *Dir, *Pkg), Ar );
							}
						}
					}
				}
				...
				{
					Ar.Logf( TEXT("No maps found [%s]"), *WildCard );
				}
			}
							...
							for( TFieldIterator<UProperty> ItP(Class); ItP; ++ItP )
								if( ItP->PropertyFlags & CPF_Localized )
									for( INT i=0; i<ItP->ArrayDim; i++ )
										if( ItP->ExportText( i, Value=Temp, Class->GetDefaultsSafe(0), ItP->GetOuter()!=Class ? Class->GetSuperClass()->GetDefaultsSafe(0) : NULL, 0 ) )
										{
					...
					else
					{
						if( It->GetClass()->ClassFlags&CLASS_Localized )
						{
							...
							for( TFieldIterator<UProperty> ItP(It->GetClass()); ItP; ++ItP )
								if( ItP->PropertyFlags & CPF_Localized )
									for( INT i=0; i<ItP->ArrayDim; i++ )
										if( ItP->ExportText( i, Value=Temp, (BYTE*)*It, It->GetClass()->GetDefaultsSafe(0), 0 ) )
										{
											const TCHAR* Key = ItP->GetName();
											if( ItP->ArrayDim!=1 )
											{
