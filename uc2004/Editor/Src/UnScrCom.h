...
{
	CPRT_None					=0,		// no reference
	CPRT_AssignValue			=1,		// we're referencing this property in order to assign a value to it
	CPRT_SimpleReference		=2,		// we're referencing this property's value in such a way that doesn't require it be initialized (if checks, iterators, etc.)
	CPRT_AssignmentReference	=3,		// we're referecning this property's value in such a away that requires it to be initialized (assigning to another property, using in a function call, etc)
	CPRT_DualReference			=4,		// we're referencing this property in a way that both changes its value and references the value (combination of CPRT_AssignmentReference and CPRT_AssignValue)
};
...
public:
	...
	:	Type(InType), ArrayDim(1), PropertyFlags(0), BitMask(0), MetaClass(NULL), ReferenceType(InRefType)
	{}
	FPropertyBase( UEnum* InEnum, EPropertyReferenceType InRefType = CPRT_None )
	:	Type(CPT_Byte), ArrayDim(1), PropertyFlags(0), Enum(InEnum), MetaClass(NULL), ReferenceType(InRefType)
	{}
	FPropertyBase( UClass* InClass, UClass* InMetaClass=NULL, EPropertyReferenceType InRefType = CPRT_None )
	:	Type(CPT_ObjectReference), ArrayDim(1), PropertyFlags(0), PropertyClass(InClass), MetaClass(InMetaClass), ReferenceType(InRefType)
	{}
	FPropertyBase( UStruct* InStruct, EPropertyReferenceType InRefType = CPRT_None )
	:	Type(CPT_Struct), ArrayDim(1), PropertyFlags(0), Struct(InStruct), MetaClass(NULL), ReferenceType(InRefType)
	{}
	...
	UBOOL MatchesType( const FPropertyBase& Other, UBOOL Identity ) const
	{
		...
		{
			//epic moelfke: added support for const out parameters
			if( ((Other.PropertyFlags & CPF_Const) && !(PropertyFlags & CPF_Const)) || !(Other.PropertyFlags & CPF_OutParm) )
				return 0;
...
public:
	...
	UProperty*	TokenProperty;		// property that corresponds to this FToken - null if this Token doesn't corresond to a UProperty
	union
	{
		// TOKEN_Const values.
	...
	: FPropertyBase(Property, InRefType), TokenProperty(Property)
	{
		StartPos = 0;
		StartLine = 0;
...
public:
	...
	{NAME_INDEX W=N.GetIndex(); ByteOrderSerialize(&W, sizeof(W)); return *this;}
	FArchive& operator<<( class UObject*& Res )
	{DWORD D = (DWORD)Res; ByteOrderSerialize(&D,sizeof(D)); return *this;}
	FArchive& operator<<( TCHAR* S )
	{Serialize(S,appStrlen(S)+1); return *this;}
	FArchive& operator<<( enum EExprToken E )
	{BYTE B=E; Serialize(&B,1); return *this;}
...
// This class conditionally writes to the script opcode stream
//
class FCondScriptWriter : public FScriptWriter
{
	// Indicates whether the stream should be written to or not
	UBOOL bEnabled;

public:
	FCondScriptWriter( class FScriptCompiler& InCompiler )
		: FScriptWriter( InCompiler ), bEnabled(1) {}
		//
		// Conditionally writes to the output stream
		//
		void Serialize( void* V, INT Length )
		{
			// Only write to the stream if the output is enabled
			if (bEnabled)
			{
				FScriptWriter::Serialize(V,Length);
			}
		}
		// Sets whether the emitter is enabled or not
		void Enable(UBOOL b) { bEnabled = b; }
};
...
{
	UProperty *property;
	INT AssignedLine,			// first line that the property is assigned a value
		ReferencedLine;			// first line that the property is referenced
	UBOOL	bReferenced,		// property is referenced in the function
			bUninitializedValue,// property is used before it has ever been assigned a value.
			bValueAssigned,		// property's value is used for something
			bValueReferenced;	// property is assigned a value somewhere in the function
	...
	{
		guard(FLocalProperty::GetLocalProperty);
...
public:
	...
	FCondScriptWriter	Writer;					// Script writer.
	...
	UProperty*		AffectorReturnProperty; //epic moelfke: property returned by affector
	...
	//epic moelfke: added bInParentheses to indicate recursive CompileExpr() calls due to an expression within parentheses
	INT				CompileExpr(const FPropertyBase RequiredType, const TCHAR* ErrorTag=NULL, FToken* ResultType=NULL, INT MaxPrecedence=MAXINT, FPropertyBase* HintType=NULL, UBOOL bInParentheses = 0);
	...
	void			CheckObscures( UStruct* Scope, FToken& Token, UBOOL bCheckLocals=0 );
	...
	TMap<UProperty*,INT,256>	LocalPropertyLineNumbers;
