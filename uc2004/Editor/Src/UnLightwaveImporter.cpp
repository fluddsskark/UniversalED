...
)
{
    ...
    if (dimension != 2)
    {
        warnf( NAME_Error, TEXT("VMAP dimension is %d (should be 2!)."), dimension );
        return;
    }
...
)
{
    ...
    while(ChunkReadPtr < ((ReadPtr + ChunkLength)-4))
    {
        ...
        if (vertexcount < 3)
        {
            warnf( NAME_Warning, TEXT("Degenerate polygon found, ignoring it! (%d verts)"), vertexcount );
...
) 
{
    ...
    while(ChunkReadPtr < ((ReadPtr + ChunkLength)-4))
    {
        ...
        if (vertexcount != 3)
        {
            warnf( NAME_Warning, TEXT("Non-triangular polygon found, ignoring it! (%d verts)"), vertexcount );
...
)
{
    ...
    if (dimension != 2)
    {
        warnf( NAME_Warning, TEXT("VMAD dimension is %d (should be 2!)."), dimension );
        return;
    }
...
)
{
    ...
    if (appStrstr(Ch,TEXT("uv2")))
    {
        ...
        for(INT j = 0; j < texturecoordcount; j++)
        {
            ...
            FLOAT newV = 1.f - GrabFloatFromArray(Data,ChunkReadPtr);
#else
            GrabFloatFromArray(Data,ChunkReadPtr);
            GrabFloatFromArray(Data,ChunkReadPtr);
#endif
            ...
            if (k == 3)
            {
                warnf( NAME_Warning, TEXT("Unmatched secondary VMAD coord!"));
            }
        }
    }
    else
    {    
        ...
        for(INT j = 0; j < texturecoordcount; j++)
        {
            ...
            if (k == 3)
            {
                warnf( NAME_Warning, TEXT("Unmatched primary VMAD coord!"));
            }
        }
    }
}
        ...
        if (ParseMaterialOption(&Ch, Texture))
        {
            ...
            if (!appStricmp(*Texture,TEXT("col")))
            {
                ...
                if (CollisionSurfaceCount > 1)
                {
                    ...
                    warnf( NAME_Error, TEXT("Extra collision surface found!"));
                }
            }
            else
            {
                ...
                {
                    BaseMaterials(surfindex) = DefaultMaterial;
                    warnf( NAME_Warning, TEXT("Couldn't find texture for surface name = %s."), *Texture );
                }
                else
...
)
{
    ...
    for(INT j = 0; j < mappingcount; j++)
    {
        ...
        {
            warnf( NAME_Error, TEXT("Too many surfaces! Smoothing groups require less than 32 surface definitions!") );
        }
...
)
{
    ...
    do
	{
        ...
        if (SubChunkTag == (INT)(('S' << 24) + ('M' << 16) + ('A' << 8) + 'N'))
        {
            ...
            {
                warnf( NAME_Warning, TEXT("Found surface name with no matching TAGS entry!") );
            }
            else
            {
...
)
{
    ...
    if (Brush == NULL)
    {
        warnf( NAME_Error, TEXT("Couldn't create brush from layer.") );
        return;
    }
...
UBOOL VerifyLWOFile(TArray<BYTE> &Data, INT &ReadPtr)
{
    ...
    {
        warnf( NAME_Error, TEXT("Lightwave file may be damaged or corrupt.") );
        return false;
    }
...
EDITOR_API UStaticMesh *CreateStaticMeshFromLWO(UObject* InOuter, FName InName, const FString &InFileName, ULevel *Level, UClass* Class)
{
    ...
    {
        warnf( NAME_Error, TEXT("Could not open %s"), *InFileName );
	    GWarn->EndSlowTask();
        return NULL;
    }
    ...
    {
        if (!GImportedBrush)
            warnf( NAME_Error, TEXT("Error reading from %s."), *InFileName );
    }
