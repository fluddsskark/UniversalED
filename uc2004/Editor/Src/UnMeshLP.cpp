...
void UEditorEngine::meshVertLODProcess( UVertMesh* Mesh, TArray<FMeshTri>& RawTris, FLODProcessInfo* LODInfo)
{
	...
	NOTE( warnf( NAME_Log, TEXT("Vertex-animated mesh LOD processing: %s"), Mesh->GetName() ); )
...
void UEditorEngine::meshSkelLODProcess( USkeletalMesh* Mesh, FLODProcessInfo* LODInfo, USkelImport* RawData)
{
	...
	{		
		...
		// scion sz -
		// default to only 2 LOD levels
		Mesh->GenerateLodModel( 0, 1.00f, 1.00f, 4, true );
		Mesh->GenerateLodModel( 1, 0.50f, 0.50f, 1, true );
		...
		// scion sz -
		// convert to GPU skinned by default
		for( int i=0; i< Mesh->LODModels.Num(); i++)
		{
			USkeletalMeshInstance* MeshInstance = (USkeletalMeshInstance*)Mesh->MeshGetInstance(NULL);
			Mesh->LODModels(i).ConstructGPUStream( MeshInstance, GPUBONES_MAX );
		}
	}		
