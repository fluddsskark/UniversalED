...
void FEditorVisibility::AssignLeaves( INT iNode, INT Outside )
{
	...
	for( int IsFront=0; IsFront<2; IsFront++ )
	{
		...
		{
			Node.iLeaf[IsFront] = Model->Leaves.AddItem( FLeaf(Model->Leaves.Num(), INDEX_NONE) );
		}
	}
	unguard;
}
...
)
{
	...
	// scion sz - check for edge-on case. If we don't then there are
	// cases where we could recurse the leaves infinitely by using 
	// invalid neighboring leaves.
	const FLOAT MIN_PLANE_DOT_EPSILON = 0.001f;
	...
	for( Portal=LeafPortals[iLeaf]; Portal!=NULL; Portal=Portal->Next(iLeaf) )
	{
		...
		if( PlaneDot<0.0f && 
			(Abs(PlaneDot) > MIN_PLANE_DOT_EPSILON) &&
			((PlaneDot > -Actor->WorldLightRadius())) )
		{
...
void FEditorVisibility::TestVisibility()
{
	...
	for( int i=0; i<Model->Leaves.Num(); i++ )
	{
		check(Model->Leaves(i).iPermeating < 0);
		if( LeafLights[i] )
		{
...
#if 0 //jg -- I removed this since it wasn't used
	...
	//scion jg -- Merge fix from Unprog
	for( int i=0; i<Level->Actors.Num(); i++ )
	{
		AActor* Actor = Level->Actors(i);
