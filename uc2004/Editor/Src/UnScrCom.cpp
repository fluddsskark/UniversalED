...
{
			/*   None      Byte                Int                 Bool              Float                Object             Name             String                                Range           	Struct           Vector               Rotator              String              */
			/*   --------  ------------------  ------------------  ----------------  -------------------  -----------------  ---------------  -------------------  ---------------  --------------- 	---------------- -------------------  -------------------- -----------------   */
...
/*          */ { CST_Max,  CST_Max,            CST_Max,            CST_Max,          CST_Max,             CST_Max,           CST_Max,         CST_Max,             CST_Max,         CST_Max,			CST_Max,         CST_Max,             CST_Max,             CST_Max,             },
/* Range    */ { CST_Max,  CST_Max,            CST_Max,            CST_Max,          CST_Max,             CST_Max,           CST_Max,         CST_Max,             CST_Max,         CST_Max,			CST_Max,         CST_Max,             CST_Max,             CST_Max,             },
...
/* String   */ { CST_Max,  CST_ByteToString,   CST_IntToString,    CST_BoolToString, CST_FloatToString,   CST_ObjectToString,CST_NameToString,CST_Max,			   CST_Max,         CST_RangeToString,	CST_Max,         CST_VectorToString,  CST_RotatorToString, CST_Max,             },
};
#undef AC
#undef TAC
...
void FScriptCompiler::EmitStackNodeLinkFunction( UFunction* Node, UBOOL ForceFinal, UBOOL Global )
{
	...
	{
		// Delegate function
		Writer << (ForceFinal ? EX_FinalDelegate : EX_DelegateFunction);
...
INT FScriptCompiler::GetToken( FToken& Token, const FPropertyBase* Hint, INT NoConsts )
{
	...
	if( (c>='A' && c<='Z') || (c>='a' && c<='z') || (c=='_') )
	{
		...
		if (!NoConsts)
		{
			...
			if( Token.TokenName==NAME_True )
			{
				Token.SetConstBool(1);
				return 1;
			}
			if( Token.TokenName==NAME_False )
			{
				Token.SetConstBool(0);
				return 1;
			}
			if( Token.Matches(TEXT("None")) )
			{
			...
			if( Token.TokenName==NAME_Rot && MatchSymbol(TEXT("(")) )
			{
			...
			if( Token.TokenName==NAME_ArrayCount )
			{
				FToken TypeToken;
			...
			if( Token.TokenName==NAME_Rng && MatchSymbol(TEXT("(")) )
			{
				// This is a range constant.
				FRange R = FRange( 0.f, 1.f );
				if(!GetConstFloat(R.Min))	appThrowf( TEXT("Missing Min component of range" ));
				if(!MatchSymbol(TEXT(","))) appThrowf( TEXT("Missing ',' in range"           ));
				if(!GetConstFloat(R.Max))	appThrowf( TEXT("Missing Max component of range" ));
				if(!MatchSymbol(TEXT(")")))	appThrowf( TEXT("Missing ')' in range"           ));
	...
	else if( c=='"' )
	{
		...
		while( (c!='"') && !IsEOL(c) )
		{
			if( c=='\\' )
			{
				...
				if( IsEOL(c) )
					break;
				//scion bcl -- modified so \n, \r, and \t in strings corespond to their C++ equivalents
				else if (c == 'n')
					c = '\n';
				else if (c == 't')
					c = '\t';
...
INT FScriptCompiler::GetConstInt( INT& Result, const TCHAR* Tag, UStruct* Scope )
{
	...
	if( Tag ) appThrowf( TEXT("%s: Missing constant integer"), Tag );
	return 0;
...
)
{
	...
	if( InName!=NAME_None )
	{
		...
		{
			if (FieldClass == UProperty::StaticClass())
			{
				for( TFieldIterator<UField,CLASS_IsAUProperty> It(Scope); It; ++It )
				{
			...
			if (FieldClass == UFunction::StaticClass())
			{
				for( TFieldIterator<UField,CLASS_IsAUFunction> It(Scope); It; ++It )
				{
					if( It->GetFName()==InName )
					{
						if( !It->IsA(FieldClass) )
			...
			if (FieldClass == UState::StaticClass())
			{
				for( TFieldIterator<UField,CLASS_IsAUState> It(Scope); It; ++It )
				{
					if( It->GetFName()==InName )
					{
						if( !It->IsA(FieldClass) )
...
void FScriptCompiler::CheckObscures( UStruct* Scope, FToken& Token, UBOOL bCheckLocals )
{
	guardSlow(FScriptCompiler::CheckObscures);

	UStruct* BaseScope = Scope->GetInheritanceSuper();

	static UBOOL bStrictChecking = ParseParam(appCmdLine(), TEXT("STRICT"));

	if ( bCheckLocals )
	{
		// Check for name conflicts on local variables
		UFunction* Func = BaseScope ? NULL : Cast<UFunction>(Scope);

		// Native functions and static functions should be ignored when checking for name conflicts inside functions
		// (functions which have no body should also be ignored, but the FUNC_Defined flag hasn't yet been set
		// when we're checking function parameters)
		if ( Func && !(Func->FunctionFlags & (FUNC_Native|FUNC_Static)) )
			BaseScope = Func->GetOwnerClass();
	}
	...
	while( BaseScope )
	{
		...
		{
			// this field's name obscures another field declared in a parent class.
			// determine if we should write a warning -

			UBOOL bIsFunction = Existing->GetClass() == UFunction::StaticClass();
			UBOOL bCheckField = !bIsFunction;
			if ( bCheckField )
			{
				// if the original field of this name has one of the following names, don't treat it as a warning
				bCheckField = 
					(Token.TokenName != TEXT("VSize")) &&
					(Token.TokenName != TEXT("Box")) &&
					(Token.TokenName != TEXT("Matrix")) &&
					(Token.TokenName != TEXT("Color")) &&
					(Token.TokenName != TEXT("Range")) &&
					(Token.TokenName != TEXT("Scale")) &&
					(Token.TokenName != TEXT("NextScript"));
			}

			// if -strict was specified on the command-line, and the original field was not a function or
			// one of the special fields we allow to be obscured, log a warning.
			if ( bStrictChecking || bCheckField )
			{
				Warn->Logf( NAME_Warning, TEXT("'%s' obscures %s '%s' defined in base class '%s'."), *Token.TokenName, Existing->GetClass()->GetName(), Existing->GetName(), BaseScope->GetName() );
			}
		}
...
UStruct* FScriptCompiler::CompileStruct( UStruct* Scope )
{
	...
	UBOOL IsNative = 0, IsExport = 0, IsTransient = 0;
	...
	for(;;)
	{
		...
		if ( StructToken.TokenName == NAME_Transient )
			IsTransient = 1;
	    else
	        break;
	}
	...
	if (IsTransient)
		Struct->StructFlags |= STRUCT_Transient;
...
)
{
	...
	//epic moelfke: added const specifier for accessing constants defined in other unrelated classes
	...
	else if( Token.Matches(NAME_Super) )
	{
		...
		RequireSymbol( TEXT("("), TEXT("'super'") );
		//epic moelfke: changed this to allow specifying a superstate instead of a superclass
		//also fixed being able to specify an invalid super name if the function existed in the specified struct
		//(for example, Super(Pawn).BeginState() in a Weapon)
		...
		while (SuperScope && SuperScope->GetFName() != SuperToken.TokenName)
		{
	...
	if( !Field )
	{
		...
		//epic moelfke: support for referencing an enum constant anywhere, not just in the class where the enum was defined
		if (!Field && PeekSymbol(TEXT(".")))
		{
			Field = (UField*)UObject::StaticFindObjectFast(UEnum::StaticClass(), NULL, Token.Identifier, 0, 1);
		}
	}
	...
	else if( Field && Cast<UProperty>(Field) )
	{
		...
		// rjp -- If RF_Public is not set on protected variables, subclasses which exist in other packages
		// can't reference the property, since the linker won't include the property in the package's export map.
		// In addition, attempting to reference a protected variable in a different object of the same class would
		// result in "Graph is linked to external private object" crash when running ucc make.
		...
		// allow classes declared 'within' other classes to access their Outer's protected members
		if ( Property->GetFlags() & RF_Protected )
		{
			UClass* OuterClass;
			for ( OuterClass = Class; OuterClass && OuterClass != UObject::StaticClass(); OuterClass = OuterClass->ClassWithin )
				if ( OuterClass->IsChildOf(Property->GetOwnerClass()) )
					break;

			if ( OuterClass == NULL || OuterClass == UObject::StaticClass() )
				appThrowf( TEXT("Can't access protected variable '%s' in '%s'"), Property->GetName(), Property->GetOwnerClass()->GetName() );
		}
		// -- rjp

		// DELTA DEFAULTS
		// full defaults required for any class defaults that are explicitly accessed
		if( Default && 
			Property->GetOuter()->IsA(UClass::StaticClass()) ) 
		{
			UClass* PropertyClass = Cast<UClass>( Property->GetOuter() );
			if( !(PropertyClass->ClassFlags & CLASS_FullDefaults) )
			{
				PropertyClass->ClassFlags |= CLASS_FullDefaults;
				if (!ParseParam(appCmdLine(),TEXT("SILENTBUILD"))) 
				{
					Warn->Logf( TEXT("Accessing default obj (Line=%d). Full defaults required for'%s'"), Token.StartLine, PropertyClass->GetName());
				}
			}
		}		
		...
		else if (Property->GetOuter()->GetClass() == UFunction::StaticClass())
		{
		...
		if ( Scope->GetClass() == UFunction::StaticClass() )
		{
			if ( LocalProperties )
			{
				...
				if ( CurLocal )
				{
					...
					// when used as an l-value, the RequiredType.Type is CPT_None.
					...
					{
						if ( ResultType.Type == CPT_ObjectReference		// if the local property we're accessing is an object variable
							&& PeekSymbol(TEXT("."))					// and we're actually accessing an internal member of the object
							&& ResultType.ArrayDim != 0 )				// and we aren't actually calling a dynamic array function on a dynamic array of objects (like .Remove, .Length, etc.)
						{
							// this is actually a reference to the internal member of a local object field
					...
					if ( RequiredType.ReferenceType == CPRT_AssignmentReference || RequiredType.ReferenceType == CPRT_DualReference )
					{
						...
						if ( !CurLocal->bValueAssigned 
							&& !(ResultType.ArrayDim == 0 && PeekSymbol(TEXT("."))) )	// if we're actually making a function call on a dynamic array, don't flag this as uninitialized 
						{
					...
					else if ( RequiredType.ReferenceType == CPRT_SimpleReference )
					{
						// special-case hack for handling post-operators.  Since post-operators cause a value
						// to be assigned, but are processed after the token, we need to switch the reference type
						// here, in case this is the only place where the value of the field is referened.
						if ( PeekSymbol(TEXT("++")) || PeekSymbol(TEXT("--")) )
						{
							// we'll also "cheat" a little and allow types which have post-operators (bytes and ints)
							// to be accessed by the post-operator without being initialized (i.e. the first time the
	...
	else if( RequiredType.Type == CPT_Delegate )
	{
		if( Field && Field->GetClass()==UFunction::StaticClass() )
		{
			...
			//epic moelfke: native final functions are not added to the function map so the FindFunction()
			//call that is used to find delegate assignments won't find them
			if ((Function->FunctionFlags & (FUNC_Native | FUNC_Final)) == (FUNC_Native | FUNC_Final))
			{
				appThrowf(TEXT("Native final function '%s' cannot be assigned to a delegate"), *Function->FriendlyName);
			}
		...
		{
			appThrowf( TEXT("Delegate assignment failed (%s): Invalid function '%s'"), *RequiredType.Function->FriendlyName, Field ? Field->GetName() : ResultType.Identifier ); 
			return 1;
		}
	}
	else if( Field && Field->GetClass()==UFunction::StaticClass() && MatchSymbol(TEXT("(")) )
	{
		...
		UFunction* Function = CastChecked<UFunction>( Field );
		GotAffector = 1;
//scion bcl - conditional compilation
		// Check for debug flags and FUNC_DebugOnly. If true, disable output so
		// the function call doesn't not get placed into the opcode stream
		if (bEmitDebugInfo == false &&
			Function->FunctionFlags & FUNC_DebugOnly)
		{
			// Disable all script output
			Writer.Enable(0);
		}
		...
		//epic moelfke: changed to a TArray to remove function parameter limit
		...
		for( It; It && (It->PropertyFlags&(CPF_Parm|CPF_ReturnParm))==CPF_Parm; ++It,++Count )
		{
			...
			ParmTokens.Add();
			//epic moelfke: added an ErrorTag for more useful error messages
		...
		{
			//epic moelfke: more useful error message when a comma was found (indicating more parameters were specified)
			if (Temp.Matches(TEXT(",")))
			{
				appThrowf(TEXT("Call to '%s': Function only takes %i parameter(s)"), Token.Identifier, (INT)Function->NumParms);
			}
			else
			{
		...
		if( It && (It->PropertyFlags & CPF_ReturnParm) )
		{
			...
			//epic moelfke: record return property so if the return value is unused we know how to destroy it
...
//scion bcl - conditional compling
		// Re-enable emitting here.
		Writer.Enable(1);
...
)
{
	...
	else if( Token.TokenName == NAME_None && RequiredType.Type == CPT_Delegate && Token.TokenType == TOKEN_Const )
	{
	...
	else if( Token.Matches(TEXT("(")) )
	{
		...
		if(!CompileExpr(RequiredType, NULL, &Token, MAXINT, 0, 1))
	...
	&&	MatchSymbol(TEXT("(")) )
	{
		...
		FToken FromType;
		CompileExpr( FPropertyBase(CPT_None,CPRT_AssignmentReference), *Token.TokenName, &FromType );
	...
	else if( CompileFieldExpr( TopNode, RequiredType, Token, Token, 1, (TopNest->Allow&ALLOW_Instance)!=0 ) )
	{
		...
		//epic moelfke: if we need an expression that returns a value, verify that we compiled one
	...
	for( ; ; )
	{
		...
		if( Token.ArrayDim==0 && MatchSymbol(TEXT(".")) )
		{
			...
			//scion rwd - array shuffle
			else if (MatchIdentifier(NAME_Shuffle))
			{
				// remember where we are in the bytecode
				FRetryPoint HighRetry; InitRetry(HighRetry);
				// write out the proper op
				Writer << EX_DynArrayShuffle;
				CodeSwitcheroo(LowRetry,HighRetry);
				// make sure we're parsing a proper expression
				RequireSymbol(TEXT("("),TEXT("'shuffle()'"));
				RequireSymbol(TEXT(")"),TEXT("'shuffle()'"));
				Token   = FToken(FPropertyBase(CPT_None));
				GotAffector = 1;
				AffectorReturnProperty = NULL;
			...
			//epic moelfke: append an array to another array of the same type
			else if (MatchIdentifier(NAME_AppendArray))
			{
				// remember where we are in the bytecode
				FRetryPoint HighRetry; InitRetry(HighRetry);
				// write the operation
				Writer << EX_DynArrayAppendArray;
				// swap the bytecode so the append code comes first
				CodeSwitcheroo(LowRetry, HighRetry);
				// compile the argument
				RequireSymbol(TEXT("("), TEXT("'AppendArray(...)'"));
				CompileExpr(Token, TEXT("'AppendArray(...)'"));
				RequireSymbol(TEXT(")"), TEXT("'AppendArray(...)'"));
...
*/		else if( Token.ArrayDim!=1 )
		...
		else if( Token.Type==CPT_ObjectReference && MatchSymbol(TEXT(".")) )
		{
			...
			if( Token.PropertyClass==UClass::StaticClass() )
			{
				...
				//epic moelfke: added const specifier for accessing constants defined in other unrelated classes
				if( PeekIdentifier(NAME_Default) || PeekIdentifier(NAME_Static) || PeekIdentifier(NAME_Const) )
				{
					// Write context token.
					FRetryPoint HighRetry; InitRetry(HighRetry);
					...
					if( !CompileFieldExpr( Token.MetaClass, RequiredType, Token, Token, 0, 0 ) )
					{
			...
			FRetryPoint ContextStart; 
			InitRetry(ContextStart);
			...
			// Insert skipover info for handling null contexts.
			FRetryPoint ContextEnd;
			InitRetry(ContextEnd);
			_WORD wSkip = TopNode->Script.Num() - ContextStart.CodeTop;
			Writer << wSkip;
	...
	{
		// Got nothing.
		//epic moelfke: don't throw an error if we failed to get an optional parameter
...
#if 1 //scion rwd - merged in tertiary operator support from Lucas Alonso of digitalo
	if( Token.Type == CPT_Bool && MatchSymbol(TEXT("?")) )
	{
		...
		//epic moelfke: use RequiredType as a hint if there's trouble figuring out the expression (fixes enums)
		CompileExpr(FPropertyBase(CPT_None,CPRT_SimpleReference), TEXT("conditional operator"), &ResultA, MAXINT, &RequiredType);
		RequireSymbol( TEXT(":"), TEXT("conditional operator") );
		// grab the current code point
		InitRetry( HighRetry );
		// calculate an offset
		_WORD SkipOffset = HighRetry.CodeTop - MidRetry.CodeTop;
		...
		//epic moelfke: use RequiredType as a hint if there's trouble figuring out the expression (fixes enums)
		CompileExpr(FPropertyBase(CPT_None,CPRT_SimpleReference), TEXT("conditional operator"), &ResultB, MAXINT, &RequiredType);
		...
		_WORD SkipOffset2 = HighRetry.CodeTop - MidRetry.CodeTop;
		// write the offset
		Writer << SkipOffset2;
		// and move it below
		CodeSwitcheroo( MidRetry, HighRetry );
	...
	//epic moelfke: if the expression is in parentheses, ignore non-matching type for now as it will be checked again
	...
	if( !bInParentheses && !RequiredType.MatchesType(Token,0) )
	{
		...
		if( RequiredType.PropertyFlags & CPF_OutParm )
		{
			...
			if( ErrorTag )
			{
				...
				{
					//epic moelfke: include ErrorTag so the message is more useful
...
void FScriptCompiler::PushNest( ENestType NestType, FName ThisName, UStruct* InNode )
{
	...
	{
		//scion rwd - use class flags to avoid casts
		for( TFieldIterator<UField,(EClassFlags)(CLASS_IsAUState|CLASS_IsAUFunction)> It(TopNode); It && It->GetOuter()==TopNode; ++It )
		{
	...
	switch( NestType )
	{
		...
		case NEST_Function:
			...
			else
			{
				...
				//epic moelfke: we never need to look for native final functions in the function map unless they're events
				if ( !(func->FunctionFlags & FUNC_Native) || !(func->FunctionFlags & FUNC_Final) ||
					(func->FunctionFlags & FUNC_Event) )
				{
...
void FScriptCompiler::CompileDirective()
{
	...
	//sion bcl - added linenumber directive to permit resetting of InputLine after include file (for example)
...
)
{
	...
	for( ; ; )
	{
		...
		else if(Specifier.Matches(NAME_Automated) )
		{
			Flags	   |= CPF_Automated | CPF_Edit | CPF_EditInline | CPF_EditInlineNotify;
			IsVariable = 1;
		}
		...
		//scion rwd - mark precache flag for properties
		else if (Specifier.Matches(NAME_Precache))
		{
			Flags     |= CPF_AutoPrecache;
			IsVariable = 1;
		}
		else
		{
	...
	else if( VarType.Matches(NAME_Array) )
	{
		...
		DWORD InnerFlags = RF_Public;

		// this clears the private/protected attributes of the array var -- rjp
//		GetVarType( Scope, VarProperty, ObjectFlags, Disallow, TEXT("'array'") );
		GetVarType( Scope, VarProperty, InnerFlags, Disallow, TEXT("'array'") );
		if( VarProperty.ArrayDim==0 )
			appThrowf( TEXT("Arrays within arrays not supported") );
		VarProperty.ArrayDim = 0;
		RequireSymbol( TEXT(">"), TEXT("'array'") );
		if ( VarProperty.Type == CPT_Struct && (Flags & CPF_Localized) )
			Warn->Logf(NAME_Warning, TEXT("'localized': Has no effect on arrays of type 'struct'"));
	}
#if 0 //NEW: ARL(ParticleSystems)
			...
			if( (Field=FindField( TempClass, ElemIdent.Identifier ))!=NULL )
			{
				...
				else if( Field->GetClass()==UStruct::StaticClass() )
				{
					...
					if ( Flags & CPF_Localized )
						Warn->Logf( NAME_Warning, TEXT("'localized': Has no effect on variables of type 'struct'") );
				}
				else
...
)
{
	...
	if( !Skip )
	{
		...
		CheckObscures(Scope, VarToken, 1);
	}
...
#if 0 //scion rwd - not used, free up some space
		//NEW: CDH... LD comments
		if (HasCommentStr)
		{
...
void FScriptCompiler::CompileAssignment( const TCHAR* Tag )
{
	...
	CompileExpr( FPropertyBase(CPT_None,CPRT_AssignValue), TEXT("Assignment"), &RequiredType );
...
//
void FScriptCompiler::CompileAffector( EPropertyReferenceType LeftRefType, EPropertyReferenceType RightRefType )
{
	...
	else if( GotAffector )
	{
		...
		//epic moelfke: if the affector returns something, that value will be unused
		//so if it requires destruction it must be explicitly destroyed
		if (AffectorReturnProperty && (AffectorReturnProperty->PropertyFlags & CPF_NeedCtorLink))
		{
			FRetryPoint HighRetry; InitRetry(HighRetry);
			Writer << EX_DestroyReturnValue;
			Writer << AffectorReturnProperty;
			// swap the code so that EX_DestroyReturnValue comes before
...
INT FScriptCompiler::CompileDeclaration( FToken& Token, UBOOL& NeedSemicolon )
{
	...
	if( Token.Matches(NAME_Class) && (TopNest->Allow & ALLOW_Class) )
	{
		...
		for( ; ; )
		{
			...
			//scion jg -- Added to be able to deprecate classes
			else if( Token.Matches(NAME_Deprecated) )
			{
				Class->ClassFlags |= CLASS_Deprecated;
			}
			//scion rwd - poolable class support
			else if (Token.Matches(NAME_Poolable))
			{
				// mark the class as poolable
				Class->ClassFlags |= CLASS_Poolable;
			}
			//scion rwd - auto precache class support
			else if (Token.Matches(NAME_Precache))
			{
				Class->ClassFlags |= CLASS_AutoPrecache;
			}
			// scion sz - skip autoload of native class
			else if( Token.Matches(NAME_NativeSkipAutoLoad) )
			{
				Class->ClassFlags |= CLASS_NativeSkipAutoLoad;
			}
			// scion sz - load full defaults of class
			else if( Token.Matches(NAME_FullDefaults) )
			{
				Class->ClassFlags |= CLASS_FullDefaults;
			}
			// scion sz - load full defaults of class and all its base classes
			else if( Token.Matches(NAME_FullDefaultsBase) )
			{
				Class->ClassFlags |= CLASS_FullDefaultsBase;
			}
			else
			{
				UngetToken(Token);
		...
		checkSlow(Class->GetFlags()&RF_Public);
		checkSlow(Class->GetFlags()&RF_Standalone);
		...
		if (SuperClass != NULL)
		{
			Class->AddDependency( SuperClass, 1 );
		}
		...
		if( SuperClass != NULL )
		{
			Class->SetPropertiesSize( SuperClass->GetPropertiesSize() );
			Class->Defaults = SuperClass->Defaults;
		}
...
//scion bcl - conditional compilation
			else if( Token.Matches(NAME_DebugOnly) )
			{
				FuncInfo.FunctionFlags |= FUNC_DebugOnly;
			}
		...
		if( !MatchSymbol(TEXT(")")) )
		{
			...
			do
			{
				...
				FPropertyBase Property(CPT_None);
				DWORD ObjectFlags;
				//epic moelfke: support for const out parameters
				...
				//epic moelfke: if this parameter is an out parm, mark function as containing out parms
		...
		else
		{
			...
			//epic moelfke: if next token is the closing bracket, this is an empty function, so don't mark it as defined
			if (MatchSymbol(TEXT("}")))
			{
				check((TopFunction->FunctionFlags & FUNC_Defined) == 0);
			}
		...
		for( INT i=NestLevel-2; i>=1; i-- )
		{
			for( TFieldIterator<UFunction> Function(Nest[i].Node); Function; ++Function )
			{
				...
				&&	((Function->FunctionFlags ^ TopFunction->FunctionFlags) & (FUNC_Operator | FUNC_PreOperator))==0 )
				{
					...
					//epic moelfke: check possible accidental omission of simulated keyword
					//note: simulated is irrelevant to Controllers because they should always be either ROLE_None or ROLE_AutonomousProxy
					if ( !(TopFunction->FunctionFlags & FUNC_Simulated) && (TopFunction->FunctionFlags & FUNC_Defined) &&
						!(TopFunction->FunctionFlags & FUNC_Static) && Class->IsChildOf(AActor::StaticClass()) && 
						!Class->IsChildOf(AController::StaticClass()) )
					{
						UBOOL bFoundSimulatedSuper = 0;
						for (UFunction* Func = TopFunction->GetSuperFunction(); Func && !bFoundSimulatedSuper; Func = Func->GetSuperFunction())
						{
							if ((Func->FunctionFlags & FUNC_Simulated) && (Func->FunctionFlags & FUNC_Defined))
							{
								warnf(NAME_Warning, TEXT("Function %s is not simulated but super function %s is"), TopFunction->GetName(), Func->GetPathName());
								bFoundSimulatedSuper = 1;
							}
						}
					}
	...
	else if( Token.Matches(NAME_Var) || Token.Matches(NAME_Local) )
	{
		...
		UBOOL bGlobalVar = Token.Matches(NAME_Var);
		DWORD Disallow;
		if( bGlobalVar )
		{
		...
		do
		{
			...
			if ( TopNest->NestType == NEST_Function )
				LocalPropertyLineNumbers.Add(newProperty, InputLine);
	...
	||	Token.Matches(NAME_Simulated) )
	{
		...
		//epic moelfke: check possible accidental omission of simulated keyword
		//note: simulated is irrelevant to Controllers because they should always be either ROLE_None or ROLE_AutonomousProxy
		if ( !(State->StateFlags & STATE_Simulated) && Class->IsChildOf(AActor::StaticClass()) && 
			!Class->IsChildOf(AController::StaticClass()) )
		{
			UBOOL bFoundSimulatedSuper = 0;
			for (UState* NextState = State->GetSuperState(); NextState && !bFoundSimulatedSuper; NextState = NextState->GetSuperState())
			{
				if (NextState->StateFlags & STATE_Simulated)
				{
					warnf(NAME_Warning, TEXT("State %s is not simulated but super state %s is"), State->GetName(), NextState->GetPathName());
					bFoundSimulatedSuper = 1;
				}
			}
		}
...
void FScriptCompiler::CompileCommand( FToken& Token, UBOOL& NeedSemicolon )
{
	...
	if( Token.Matches(NAME_Switch) )
	{
		...
		Writer << EX_Switch;
		FRetryPoint LowRetry; InitRetry(LowRetry);
		CompileExpr( FPropertyBase(CPT_None,CPRT_SimpleReference), TEXT("'Switch'"), &TopNest->SwitchType );
	...
	else if( Token.Matches(NAME_Return) )
	{
		...
		if( Return )
		{
			...
			// reference type should ideally be AssignmentReference, but that causes
...
//			FPropertyBase Property(Return,CPRT_AssignmentReference);
	...
	else if( Token.Matches(NAME_If) )
	{
		...
		CompileExpr( FPropertyBase(CPT_Bool,CPRT_SimpleReference), TEXT("'If'") );
		RequireSymbol( TEXT(")"), TEXT("'If'") );
	...
	else if( Token.Matches(NAME_While) )
	{
		...
		CompileExpr( FPropertyBase(CPT_Bool, CPRT_SimpleReference), TEXT("'While'") );
		RequireSymbol( TEXT(")"), TEXT("'While'") );
	...
	else if(Token.Matches(NAME_For))
	{
		...
		RequireSymbol( TEXT(";"), TEXT("'For'") );
			...
			CompileExpr( FPropertyBase(CPT_Bool, CPRT_SimpleReference), TEXT("'For'") );
		RequireSymbol( TEXT(";"), TEXT("'For'") );
...
void FScriptCompiler::CompileSecondPass( UStruct* Node )
{
	...
	if( Node==Class && Node->Line!=INDEX_NONE )
	{
		...
		while( 1 )
		{
            ...
            //scion rwd - mongo's multicast replication
				...
				if( !Found )
				{
					...
					for( TFieldIterator<UFunction> Function(Class); Function && Function.GetStruct()==Class; ++Function )
					{
						if( Function->GetFName()==VarName )
						{
							...
							Found                     = 1;
                            //scion rwd - mongo's multicast replication
                            if (FunctionFlags & FUNC_Multicast) {
                                debugf(TEXT("Multicast function: %s"),Function->GetName());
                            }
							break;
						}
					}
...
)
{
	...
	try
	{
		...
		if( Pass == 0 )
		{
			...
			//epic moelfke: if no state is marked auto, mark class as auto for faster GotoState()
			UBOOL bFoundAuto = 0;
		...
		for( TArray<UClass*>::TIterator It(AllClasses); It; ++It )
        {
			if( (*It)->GetSuperClass()==Class)
            {
                ...
                for (INT nameIdx=0; nameIdx<(*It)->DependentOn.Num(); nameIdx++)
                {
                    ...
                    {
						//epic moelfke: make sure to parse superclasses of the class we're dependant on first
						TArray<UClass*> UnparsedSuperClasses;
						// create a list of unparsed superclasses
						for (UClass* TestClass = clsTmp->GetSuperClass(); TestClass && !(TestClass->ClassFlags & CLASS_Parsed); TestClass = TestClass->GetSuperClass())
						{
							UnparsedSuperClasses.AddItem(TestClass);
						}
						// iterate through the list and parse them all
						for (INT i = 0; i < UnparsedSuperClasses.Num(); i++)
						{
							if (!ParseScripts(AllClasses, Compiler, UnparsedSuperClasses(i), MakeAll, Booting, MakeSubclasses))
							{
								return 0;
							}
						}

						// now parse the actual class we're dependant on
...
static UBOOL ImportPropertiesScriptsWorker( TArray<UClass*>& AllClasses, FScriptCompiler& Compiler, UClass* Class, UBOOL MakeSubclasses )
{
	...
	if( Class->ClassFlags & CLASS_NeedsDefProps )
	{
		...
		UBOOL ImportSuccess = !Class->DefaultPropText || ImportProperties( Class, Class->GetDefaultsSafe(0), (ULevel*)NULL, *Class->DefaultPropText, Class->GetOuter(), GWarn, 0 )!=NULL;
...
UBOOL UEditorEngine::MakeScripts( UClass* BaseClass, FFeedbackContext* Warn, UBOOL MakeAll, UBOOL Booting, UBOOL MakeSubclasses )
{
	...
	Compiler.UseLineNumbers = ParseParam(appCmdLine(), TEXT("lineinfo"));
	Compiler.InDefaultPropContext = 0;
	...
	for( TArray<UClass*>::TIterator It(AllClasses); It; ++It )
	{
		...
		&&	!((*It)->ClassFlags & CLASS_Parsed) )
		{
			if( (*It)->Defaults.Num() && (*It)->DefaultPropText==TEXT("") )
			{
				...
				ExportProperties( Ar, *It, (*It)->GetDefaultsSafe(0), 0, (*It)->GetSuperClass(), (*It)->GetSuperClass() ? (BYTE*)(*It)->GetSuperClass()->GetDefaultObject() : NULL );
	...
	// scion sz - mark classes with a superclass that has CLASS_FullDefaultsBase as CLASS_FullDefaults
	guard(MarkFullDefaultsFromBase);
	for( TArray<UClass*>::TIterator It(AllClasses); It; ++It )
	{
		// iterate super classes looking for full defaults base flag
		for( UClass* pClass=(*It); pClass; pClass=pClass->GetSuperClass() )
		{
			// found matching parent class w/ flag
			if( pClass->ClassFlags&CLASS_FullDefaultsBase )
			{
                // class should have full defaults				
                (*It)->ClassFlags |= CLASS_FullDefaults;
				debugf( NAME_DevCompile, TEXT("Marking full defaults for class=[%s] because of parent class=[%s]"),
					(*It)->GetName(), pClass->GetName() );
				break;
			}
		}
	}
