...
class UAnalyzeScriptCommandlet : public UCommandlet
{
	...
	INT Main(const TCHAR *Parms)
	{
		...
		TMultiMap<FName,FString> *section = GConfig->GetSectionPrivate(TEXT("Editor.EditorEngine"),0,1,TEXT("Avaris.ini"));
		check(section != NULL && "Failed to find editpackages, did you specify the correct ini?");
		section->MultiFind(FName(TEXT("EditPackages")),editPkgs);
