...
void UTrueTypeFontFactory::StaticConstructor()
{
	...
	Compression = TEXF_RGBA8;
	new(GetClass(),TEXT("Compression"),			RF_Public)UIntProperty (CPP_PROPERTY(Compression	),	TEXT(""),	CPF_Edit );
	unguard;
}
...
)
{
	...
	if( UseFiles || UseRange )
	{
		...
		TArray<BYTE> ValidChars;
		ValidChars.AddZeroed(65536);
	...
	HFONT F = TCHAR_CALL_OS(
		CreateFont(nHeight, 0, 0, 0, Style, Italic, Underline, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, AntiAlias ? ANTIALIASED_QUALITY : NONANTIALIASED_QUALITY, VARIABLE_PITCH, *FontName),
		CreateFontA(nHeight, 0, 0, 0, Style, Italic, Underline, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, AntiAlias ? ANTIALIASED_QUALITY : NONANTIALIASED_QUALITY, VARIABLE_PITCH, TCHAR_TO_ANSI(*FontName)) );
	if( !F )
	{
	...
	for( INT Ch=0; Ch<Font->Characters.Num(); Ch++ )
	{
		...
		// (unless it's the null character - we always need that one, as it will be used for displaying
		// character that are not in the font)
		if( Ch != NULLCHARACTER && Ch<256 && Chars!=TEXT("") && (!Ch || !appStrchr(*Chars, Ch)) )
			continue;
		...
		// hack for the ghost in the machine
		if ( Size.cx == 0 )
		{
			debugf(TEXT("Character %i has zero width (%s)"), Ch, *FontName);
			if ( Ch == 133 )
				Size.cx = 4;
		}
...
UTexture* UTrueTypeFontFactory::CreateTextureFromDC( UFont* Font, DWORD Indc, INT Height, INT TextureNum )
{
	...
	if ( Compression != TEXF_RGBA8 )
		Texture->Compress(ETextureFormat(Compression));
