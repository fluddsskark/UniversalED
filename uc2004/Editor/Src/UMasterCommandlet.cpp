...
class UMasterCommandlet : public UCommandlet
{
	...
	void LocalCopyFile( INT& CDNum, const TCHAR* Dest, const TCHAR* Src, DWORD Flags, UBOOL Compressed, DWORD Align )
	{
		...
		else
		{
				...
				if( CDNum == 0 )
				{
					if( GAutoNextCD )
					{
						...
						if( Compressed )
						{
							...
							warnf( TEXT("   Copying %s to %s"), Src, *TempFile );
			...
			if( MoveTemp )
			{		
				warnf( TEXT("   Compressed to %d%%"), 100 * GFileManager->FileSize(*TempFile) / GFileManager->FileSize(Src) );
				warnf( TEXT("   Moving %s to %s"), *TempFile, *FullDest );
				if( !GFileManager->Move( *FullDest, *TempFile, 1, 1, 0 ) )
					appErrorf( TEXT("Failed to move %s to %s"), *TempFile, *FullDest );
			}
			else
			{
				warnf( TEXT("   Copying %s to %s"), Src, *FullDest );
	...
	UBOOL DeltaCode( const TCHAR* RefFilename, const TCHAR* MasterFilename, const TCHAR* SrcFilename )
	{
		...
		warnf( TEXT("   Delta compressing %s to %s"), MasterFilename, *(GSrcPath*SrcFilename) );
		warnf( TEXT("      Relative to %s"), RefFilename );
		...
		{
			warnf( TEXT("   Examining cached delta %s"), *(GSrcPath*SrcFilename) );
			TArray<BYTE> Test;
		...
		if( !Done )
		{
			...
			warnf( TEXT("      Result size %i (%5.3f%%) Time = %5.2f Min"), Data.Num(), 100.0*Data.Num()/New.Num(), (appSeconds()-StartTime)/60.0 );
	...
	void UpdateGroup( FString MasterPath, const TCHAR* Group, TMultiMap<FName,FString>& Map )
	{
		guard(UpdateGroup);
		warnf( TEXT("   Processing group %s"), Group );
		TMultiMap<FString,FString> AllFiles;
		{for( TMultiMap<FName,FString>::TIterator It(Map); It; ++It )
		{
			if( It.Key()==FName(TEXT("File")) || It.Key()==FName(TEXT("Copy")) )
			{
				// Expand wildcard.
				...
				{
					warnf( TEXT("   Expanding wildcard %s"), *Master );
	...
	// Copy a group.
	void CopyGroup( FString MasterPath, const TCHAR* Group, TMultiMap<FName,FString>& Map )
	{
		guard(CopyGroup);
		warnf( TEXT("   Copying group %s"), Group );

		INT CDNum = 0;
		if( GMultiCD )
		{
			FString* CDNumPtr = Map.Find( FName(TEXT("CDNumber")) );
			...
			if( CDNumPtr && GAutoNextCD )
			{
				...
				CDNum = 0;
				Map.Set( FName(TEXT("CDNumber")), appItoa(GCurrentCDNum) );
			}
			else
				Map.Set( FName(TEXT("CDNumber")), appItoa(CDNum) );
		}
	...
	void ProcessGroup( FString MasterPath, const TCHAR* File, const TCHAR* Group, void(UMasterCommandlet::*Process)( FString MasterPath, const TCHAR* Group, TMultiMap<FName,FString>& Map ) )
	{
		guard(ProcessGroup);
		TMultiMap<FName,FString>* Map = GConfig->GetSectionPrivate( Group, 0, 1, File );
		if( !Map )
	...
	INT Main( const TCHAR* Parms )
	{
		...
		warnf( TEXT("Using config: %s"), *ConfigBase );
			...
			if( Pos>=0 )
			{
				...
				if( Ext!=MANIFEST_EXT )
				{
					...
					if( List(i)!=GConfigFile && GConfig->GetString(TEXT("Setup"),TEXT("LocalProduct"),Str,*List(i)) )
					{
						...
						warnf( TEXT("   Copying manifest %s to %s"), *List(i), *Dest );
		...
		if( GArchiveData )
		{
			warnf( TEXT("   Saving archive: %s"), *(GSrcPath * GArchive) );
...
class UUpdateUModCommandlet : public UCommandlet
{
	...
	INT Main( const TCHAR* Parms )
	{
		...
		warnf( TEXT("Loading Archive: %s"), *UModFile );
		  ...
		  )
		{
			if( Command.Caps()==TEXT("LIST") )
			{
				...
				warnf( TEXT("") );
				warnf( TEXT("%-30s%10s%10s"), TEXT("Filename"), TEXT("Size"), TEXT("Offset") );
				warnf( TEXT("%-30s%10s%10s"), TEXT("--------"), TEXT("----"), TEXT("------") );

				for( INT i=0; i<SrcHdr._Items_.Num(); i++ )
					warnf( TEXT("%-30s%10d%10d"), *SrcHdr._Items_(i)._Filename_, SrcHdr._Items_(i).Size, SrcHdr._Items_(i).Offset );
			}
			else
			{
				...
				else
				{
					...
					if( Command.Caps()==TEXT("DELETE") || Command.Caps()==TEXT("REPLACE") )
					{
						for( INT i=0; i<SrcHdr._Items_.Num(); i++ )
						{
								...
								SrcHdr._Items_(i)._Filename_.Caps().Right(File.Len()+1) == FString(TEXT("\\"))+File.Caps() )
							{
								...
								//warnf( TEXT("UpdatePath is %s, file is %s"), *UpdatePath, *File );
								break;
							}			
						}
					}
					...
					warnf( TEXT("Saving archive: %s"), *UModFile );
