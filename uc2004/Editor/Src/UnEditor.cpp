...
void UEditorEngine::Tick( float DeltaSeconds )
{
	...
	// Update audio.
	if (Audio && Audio->GetViewport())
	{
		FUNC_CLOCK(AudCyc,GStats.DWORDStats(GEngineStats.STATS_Game_AudioTickCycles));
...
)
{
	...
	while( ParseLine( &Data, StrLine, ARRAY_COUNT(StrLine) ) )
	{
		...
		else if( GetBEGIN(&Str,TEXT("Object")))
		{
			...
			UClass*	SubObjectClass;
			UObject* SubObjectOuter = NULL;
			ParseObject<UObject>(Str,TEXT("Outer="),SubObjectOuter,ANY_PACKAGE);
			if( !SubObjectOuter )
				SubObjectOuter = GEditor->Bootstrapping ? ObjectClass : InParent;
			if(	ParseObject<UClass>(Str,TEXT("Class="),SubObjectClass,ANY_PACKAGE) )
			{
				...
				if( Parse(Str,TEXT("Name="),SubObjectName,NAME_SIZE) )
				{
					...
					{
						//scion rwd - allow forced rename
						if( GEditor->Bootstrapping && appStrfind(Str,TEXT("Rename")) == NULL )
		...
		{
			UBOOL bSubObject = InParent->IsA(UClass::StaticClass());
			...
			if( *Str )
			{
				...
				if( *Str=='(' || *Str=='[' )
				{
					...
					{
						// Resolve const reference in default properties subscript
						TCHAR TempToken[MAX_STRING_CONST_SIZE], *Pos = TempToken;

						while( *Str && *Str!=')' && *Str!=']' && Pos - Start < MAX_STRING_CONST_SIZE)
							*Pos++ = *Str++;
						*Pos=0;

						UConst* Const = FindField<UConst>( bSubObject ? Cast<UClass>(InParent) : ObjectClass, TempToken );
						if ( Const == NULL )
						{
							Warn->Logf(NAME_ExecWarning, TEXT("Invalid subscript in defaultproperties (must specify digit or const): %s"), StrLine);
							continue;
						}
				...
				//scion rwd - transient serialization warning
				if (Property->PropertyFlags & CPF_Transient)
				{
					Warn->Logf(NAME_Error, TEXT("%s: Attempting to assign default value to transient property: %s"), ObjectClass->GetPathName(), Property->GetName());
				}
				...
				//scion rwd - limited multi-line support
				FString FullText;
				if (*Str == '{')
				{
					//debugf(TEXT("Starting multi-line import for property %s"),Property->GetName());
					INT BracketCnt = 0;
					UBOOL bInString = false; //epic moelfke: true if our current position is inside a string value
					...
					do
					{
						...
						{
							// parse a new line
							if (ParseLine(&Data,StrLine,ARRAY_COUNT(StrLine)))
							{
								Str = StrLine;
							}
							else
						...
						{
							//epic moelfke: flip the "we are in a string" switch
							bInString = !bInString;
						}
						// add this character to our end result
						//epic moelfke: if we're currently inside a string value, allow the space character
						if ( (bInString || Char != ' ') &&
							Char != '\t' &&
							Char != '\n' &&
							Char != '\r' &&
				...
				if( appStricmp(Property->GetName(),TEXT("Name"))!=0 )
				{
					...
					{
						//scion rwd - change all '.' to '-'
						TCHAR* tmpStr = (TCHAR*)Str;
					...
					*/
					...
					if (Index > -1 && Property->IsA(UArrayProperty::StaticClass())) //set single dynamic array element
					{
						...
						//epic moelfke: check for double quote when importing single element of an array of strings
						if (ArrayProp->Inner->IsA(UStrProperty::StaticClass()) && (!l || *Str != '"' || Str[l-1] != '"'))
						{
							Warn->Logf(NAME_Error, TEXT("%s: Missing '\"' in string default properties : %s"), ObjectClass->GetPathName(), StrLine);
						}
						if (Index>=Array->Num())
					...
					else
					{
						...
						const TCHAR* Result = Property->ImportText( Str, Object + Property->Offset + Index*Property->ElementSize, PPF_Delimited | PPF_CheckReferences  );
						UBOOL bFailedImport = (Result == NULL || Result == Str);
						if ( bFailedImport )
						{
							// First, attempt to find a named constant
							UConst* Const = FindField<UConst>( bSubObject?Cast<UClass>(InParent):ObjectClass, Str );
							if ( Const )
							{
								Result = Property->ImportText( *Const->Value, Object + Property->Offset + Index*Property->ElementSize, PPF_Delimited | PPF_CheckReferences  );
								bFailedImport = (Result == NULL || Result == *Const->Value);
							}

							if ( bFailedImport )
								Warn->Logf(NAME_Warning, TEXT("%s%s%s: Invalid property value in defaults: %s"), bSubObject ? InParent->GetName() : TEXT(""), bSubObject ? TEXT(".") : TEXT(""), ObjectClass->GetName(), StrLine);
						}
					}
				}
			}
