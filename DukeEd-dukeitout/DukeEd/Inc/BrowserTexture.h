/*=============================================================================
	...
	Revision history:
		...
		* Edited by @Yockey to force software rendering in browser unless specified through DukeEd.ini to use D3DDRV

    Work-in-progress todo's:
	- needs ability to bulk export texture packages to folders, subfolders containing package categories
...
class WDlgNewTexture : public WDialog
{
	...
	void OnInitDialog()
	{
		...
		WidthCombo.AddString( TEXT("512") );
		WidthCombo.AddString( TEXT("768") );
		WidthCombo.AddString( TEXT("1024") );
		WidthCombo.AddString( TEXT("2048") );
		WidthCombo.SetCurrent(12);
		...
		HeightCombo.AddString( TEXT("512") );
		HeightCombo.AddString( TEXT("768") );
		HeightCombo.AddString( TEXT("1024") );
		HeightCombo.AddString( TEXT("2048") );
		HeightCombo.SetCurrent(12);
...
class WBrowserTexture : public WBrowser
{
	...
	void OnCreate()
	{
		...
		// Look in the DukeEd.ini for the renderer to use for the texture browser
		FString BTDevice;
		TCHAR bt_chName[20];
		appSprintf(bt_chName, TEXT("Texture Browser"));															//look in [Texture Browser] section
		if (!GConfig->GetString(bt_chName, TEXT("Device"), BTDevice, TEXT("DukeEd.ini") ) || !BTDevice.Len())	//default to software on errors
			BTDevice = TEXT("SoftDrv.SoftwareRenderDevice");
		...
		//try the texture browser renderer specified in the INI setting and revert to software if that fails
		pViewport->TryRenderDevice(*BTDevice, pViewport->SizeX, pViewport->SizeY, INDEX_NONE, 0);
		if (!pViewport->RenDev)
			pViewport->TryRenderDevice(TEXT("SoftDrv.SoftwareRenderDevice"), pViewport->SizeX, pViewport->SizeY, INDEX_NONE, 0);
	...
	void ReloadTexturePackages(void)
	{
		TCHAR l_chCmd[1024];
		FString Package = pComboPackage->GetString(pComboPackage->GetCurrent());
		...
		for (INT x = 0; x < StringArray.Num(); x++)
		{
			...
			appSprintf(l_chCmd, TEXT("OBJ LOAD FILE=\"%s"), *Package);
			GEditor->Exec(l_chCmd);
			RefreshGroups();
			return;
		}
