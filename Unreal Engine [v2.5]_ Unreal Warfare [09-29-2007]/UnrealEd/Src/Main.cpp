...
INT WINAPI WinMain( HINSTANCE hInInstance, HINSTANCE hPrevInstance, char* InCmdLine, INT nCmdShow )
{
	...
	try
	{
		...
		appInit( TEXT("UnrealEngine2"), GetCommandLine(), &Malloc, &Log, &Error, &Warn, &FileManager, FConfigCacheIni::Factory, 1 );
		...
		if( !GConfig->GetString( TEXT("URL"), TEXT("MapExt"), GMapExt, TEXT("UnrealEngine2.ini") ) )		GMapExt = TEXT("unr");
