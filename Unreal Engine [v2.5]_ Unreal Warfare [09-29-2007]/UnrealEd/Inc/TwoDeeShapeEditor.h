	...
	void ProcessRevolve( INT TotalSides, INT Sides )
	{
		...
		for( INT shape = 0 ; shape < m_shapes.Num() ; ++shape )
		{
			...
			for( INT side = 0 ; side < Sides ; ++side )
				for( INT vtidx = 0 ; vtidx < m_shapes(shape).Verts.Num() ; ++vtidx )
				{
					Cmd += TEXT("Begin Polygon Flags=0\n");

					FVector *pvtx, *pvtxPrev;

					pvtxPrev = &( m_shapes(shape).Verts( (vtidx ? vtidx - 1 : m_shapes(shape).Verts.Num() - 1 ) ) );
					pvtx = &( m_shapes(shape).Verts(vtidx) );
