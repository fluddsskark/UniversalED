...
void ConvertToType( UMeshAnimation* Anim, INT MoveIndex, INT AnimCompressMethod )
{
	...
	{				
		...
		if( LastKeyIndex == 0 ) 
		{			
			Anim->Moves(MoveIndex).FlexTracks(b).AllocateTrack( ADT_Static );

			FQuat NewOrientation = Anim->Moves(MoveIndex).AnimTracks(b).KeyQuat(0);
			NewOrientation.Normalize();
			if( NewOrientation.W < 0)
			{
				NewOrientation.X = -NewOrientation.X;
				NewOrientation.Y = -NewOrientation.Y;
				NewOrientation.Z = -NewOrientation.Z;
				NewOrientation.W = -NewOrientation.W;
			}				
			
			((FlexTrackStatic*)(Anim->Moves(MoveIndex).FlexTracks(b).FlexTrackPtr))->Orientation = FQuatPacked( NewOrientation );
			((FlexTrackStatic*)(Anim->Moves(MoveIndex).FlexTracks(b).FlexTrackPtr))->Position    = Anim->Moves(MoveIndex).AnimTracks(b).KeyPos(0);
		}
		else
		{
			// Special case - only one position key.
			if( Anim->Moves(MoveIndex).AnimTracks(b).KeyPos.Num() == 1 )
			{				
				...
				((FlexTrack48RotOnly*)(Anim->Moves(MoveIndex).FlexTracks(b).FlexTrackPtr))->Position = TempPos;				
